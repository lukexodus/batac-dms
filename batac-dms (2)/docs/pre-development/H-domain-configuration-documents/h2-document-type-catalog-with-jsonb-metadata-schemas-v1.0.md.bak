...
- [L138–L940] JSONB Metadata Schemas — Per Document Type — Introduction to JSON Schema validation configurations for the metadata column of all document types.
  ...
  - [L621–L773] 6. Document Request Form (`DOCUMENT_REQUEST_FORM`) — Requester info, requested document arrays, fee-based payment placeholders, and VM/Secretary approval flags.
...
⁵ Phase 1 citizen-facing features (Citizen Complaint, Document Request Form) are created under the `portal` schema because Part 11.9 assigns `complaints` and `citizen_requests` to `schema: portal`. In Phase 1 the `portal` schema is partially initialized: only complaint and request tables are live. Full portal module activation is Phase 3. [Inference — consistent with Part 11.9; Part 13 roadmap]
...
The dual approval requirement (VM + SP Secretary) is [Inference] represented here as two boolean flags in JSONB. An alternative is to model these as two explicit `approval` steps in the workflow engine (B4 §4.2), which would be more consistent with the platform's workflow pattern. Confirm the modeling choice with the workflow engine design before implementing.

```json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "type": "object",
  "required": ["requester", "documents_requested", "access_mode", "approval_status"],
  "additionalProperties": false,
  "properties": {
    ...
    "approval_status": {
      ...
      "description": "Aggregate approval status. [Inference] Approval requires both Vice Mayor AND SP Secretary. [Confirmed — Part 4.15] 'approved' is only set when both approvals are recorded.",
      "default": "pending"
    },

    "approved_by_vm": {
      "type": "boolean",
      "description": "[Inference] Vice Mayor approval flag. Both VM and SP Secretary must approve before approval_status transitions to 'approved'. If modeled as workflow approval steps in B4 instead, these two flags are redundant — confirm modeling choice before implementing.",
      "default": false
    },

    "approved_by_sp_secretary": {
      "type": "boolean",
      "description": "[Inference] SP Secretary approval flag.",
      "default": false
    }
...
**9. Document Request Form — approval modeling decision required**

The `approved_by_vm` and `approved_by_sp_secretary` flags in the Document Request Form JSONB are [Inference]. If the Document Request Form goes through the workflow engine (two sequential `approval` steps — one for VM, one for SP Secretary), these JSONB flags are redundant with workflow step completion records and should be removed to avoid inconsistency. If the form uses a lightweight ad-hoc approval path outside the workflow engine, the JSONB flags are appropriate. Confirm this modeling choice before Phase 1 implementation.
