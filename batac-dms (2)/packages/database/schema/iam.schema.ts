...
export const users = iamSchema.table(
  ...
  (table) => [
    ...
    check('users_status_check', sql`${table.status} IN ('active','inactive','suspended','deactivated')`),
  ]
);
...
export const credentials = iamSchema.table(
  ...
  (table) => [
    unique('uq_credentials_user').on(table.userId),
  ]
);
...
export const sessions = iamSchema.table(
  ...
  (table) => [
    ...
    check('ck_sessions_termination_consistency', sql`(${table.terminatedAt} IS NULL) = (${table.terminationReason} IS NULL)`),
    check('sessions_termination_reason_check', sql`${table.terminationReason} IN ('logout','inactivity','forced','replaced','expired','lock')`),
...
export const refreshTokens = iamSchema.table(
  ...
  (table) => [
    ...
    check('refresh_tokens_revocation_reason_check', sql`${table.revocationReason} IN ('logout','reuse_detected','forced','family_revoked','replaced')`),
...
export const roles = iamSchema.table(
  ...
  (table) => [
    ...
    check('roles_type_code_check', sql`${table.typeCode} IN ('platform_admin','document_processor','sys_admin','auditor','citizen')`),
  ]
);
...
export const rolePermissions = iamSchema.table(
  ...
  (table) => [
    ...
    check('role_permissions_decision_check', sql`${table.decision} IN ('allow','deny','conditional')`),
    check('ck_role_permissions_condition_required', sql`${table.decision} <> 'conditional' OR ${table.conditionReference} IS NOT NULL`),
  ]
);
...
export const roleAssignments = iamSchema.table(
  ...
  (table) => [
    check('ck_role_assignments_revocation_consistency', sql`(${table.revokedAt} IS NULL) = (${table.isActive} = true)`),
    uniqueIndex('uq_role_assignments_active')
      .on(table.userId, table.roleId, sql`coalesce(${table.officeScopeId}, '00000000-0000-0000-0000-000000000000'::uuid)`)
