...
// Grants generated by the City Administrator to allow migrating a stuck workflow
// instance to a new definition version (B4 invariant #8).
// ─────────────────────────────────────────────────────────────────────────────
export const adminApprovalGrants = workflowSchema.table(
  'admin_approval_grants',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    instanceId: uuid('instance_id')
      .notNull()
      .references(() => instances.id),
    newDefinitionVersionId: uuid('new_definition_version_id')
      .notNull()
      .references(() => definitionVersions.id),
    /** logical FK → iam.users.id (cross-schema) */
