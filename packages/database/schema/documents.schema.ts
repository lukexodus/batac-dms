...
export const documentTypes = documentsSchema.table(
  "document_types",
  {
    ...
    owningModule: text("owning_module")
      .$type<"workflow" | "organization" | "portal">()
      .notNull(),
    /**
    ...
    classificationDefault: text("classification_default")
      .$type<"public" | "internal" | "confidential" | "restricted">()
      .notNull(),
    publicVisibilityRule: text("public_visibility_rule")
      .$type<
...
export const numberSeries = documentsSchema.table(
  "number_series",
  {
    ...
    seriesType: text("series_type")
      .$type<"legislative" | "administrative">()
      .notNull(),
    /** '1' = active in Phase 1; '1b' = Phase 1B (seeded inactive, activated later). */
    phase: text("phase").$type<"1" | "1b">().notNull().default("1"),
    prefix: text("prefix"),
    /**
  ...
  (table) => [
    ...
    check(
      "documents_series_type_check",
      sql`${table.seriesType} IN ('legislative', 'administrative')`,
    ),
    check("documents_phase_check", sql`${table.phase} IN ('1', '1b')`),
  ],
);
...
export const numbers = documentsSchema.table(
  "numbers",
  {
    ...
    numberType: text("number_type")
      .$type<"preliminary" | "final" | "control">()
      .notNull(),
...
export const versions = documentsSchema.table(
  "versions",
  {
    ...
    scanQualityCategory: text("scan_quality_category").$type<
      "good" | "fair" | "poor"
    >(),
...
export const signatures = documentsSchema.table(
  ...
  (table) => [
    check(
      "documents_signature_type_check",
...
export const documentSponsorships = documentsSchema.table(
  ...
  (table) => [
    ...
    check(
      "documents_sponsorship_type_check",
...
export const panlalawiganReviews = documentsSchema.table(
  ...
  (table) => [
    ...
    check(
      "documents_outcome_check",
      sql`${table.outcome} IS NULL OR ${table.outcome} IN ('valid', 'valid_in_part', 'returned', 'operative_in_its_entirety', 'deemed_approved')`,
    ),
