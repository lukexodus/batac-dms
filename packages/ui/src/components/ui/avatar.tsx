 ...
 */
const AvatarName = React.forwardRef<React.ElementRef<typeof AvatarPrimitive.Root>, AvatarNameProps>(
  ({ name, size = 'md', className, ...props }, ref) => {
    ...
    return (
      <Avatar ref={ref} size={size} className={className} {...props}>
        <AvatarFallback className={cn(colorClass, 'font-semibold text-white', textSizeClass)}>
          {initials}
        </AvatarFallback>
      </Avatar>
    );
