export async function validateDefinitionForPublish(versionId, deps) {
    const { steps, transitionRules } = await deps.workflowRepository.getStepsAndRulesForValidation(versionId);
    const errors = [];
    // 1. Start step check
    ...
    for (const step of steps) {
        ...
        if (step.stepType === 'approval') {
            const allowedOutcomes = config['allowed_outcomes'] || [];
            // 3. MISSING_LAPSE_TRANSITION
            if (allowedOutcomes.includes('LAPSED')) {
                const hasLapsedRule = transitionRules.some((r) => r.fromStepId === step.id && r.outcomeFilter === 'LAPSED');
                if (!hasLapsedRule) {
                    errors.push({
            ...
            if (allowedOutcomes.includes('DEEMED_APPROVED')) {
                const hasDeemedRule = transitionRules.some((r) => r.fromStepId === step.id && r.outcomeFilter === 'DEEMED_APPROVED');
                if (!hasDeemedRule) {
                    errors.push({
            ...
            for (const code of allowedOutcomes) {
                if (code === 'LAPSED' || code === 'DEEMED_APPROVED')
                    continue;
                const hasMatchingRule = transitionRules.some((r) => r.fromStepId === step.id &&
        ...
        if (step.stepType === 'multi_referral') {
            ...
            const hasBypassRule = transitionRules.some((r) => r.fromStepId === step.id && r.outcomeFilter === 'BYPASSED_CERTIFIED_URGENT');
            if (!hasBypassRule) {
                errors.push({
