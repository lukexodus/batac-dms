...
import { cancelInstance, bypassStep, migrateInstance, reverseMigration } from './admin-operations.js';
...
describe('Admin Operations', () => {
  ...
  beforeEach(() => {
    ...
    mockDeps = {
      ...
      workflowRepository: {
        ...
        createWorkflowEvent: vi.fn().mockResolvedValue({ id: 'mock-event-id', createdAt: new Date() }),
  ...
  describe('cancelInstance', () => {
    it('throws VALIDATION_FAILED if reason is empty', async () => {
      await expect(cancelInstance('inst-1', 'admin-1', '   ', mockDeps)).rejects.toThrowError('cancellation reason must not be empty');
      await expect(cancelInstance('inst-1', 'admin-1', '', mockDeps)).rejects.toThrowError('cancellation reason must not be empty');
    });

    it('propagates already-terminal error from updateInstanceStatus', async () => {
      mockDeps.workflowRepository.updateInstanceStatus.mockRejectedValue(new Error('Cannot update status of a completed workflow instance.'));
      
      await expect(cancelInstance('inst-1', 'admin-1', 'reason', mockDeps)).rejects.toThrowError('Cannot update status');
    });

    it('cancels instance and updates step instances', async () => {
      await cancelInstance('inst-1', 'admin-1', 'Testing cancel', mockDeps);

      expect(mockDeps.workflowRepository.updateInstanceStatus).toHaveBeenCalledWith('inst-1', 'cancelled', expect.any(Date), mockTrx);
      expect(mockDeps.workflowRepository.cancelActiveAndPendingStepInstancesForInstance).toHaveBeenCalledWith('inst-1', mockTrx);
  ...
  describe('bypassStep', () => {
    it('throws VALIDATION_FAILED if comment or outcomeCode is empty', async () => {
      await expect(bypassStep('step-1', 'admin-1', 'reason', '   ', 'OUTCOME', mockDeps)).rejects.toThrowError('bypass comment must not be empty');
      await expect(bypassStep('step-1', 'admin-1', 'reason', 'comment', '', mockDeps)).rejects.toThrowError('bypass outcomeCode must not be empty');
    });

    it('throws if step instance is not active', async () => {
      mockDeps.workflowRepository.getStepInstanceById.mockResolvedValue({ id: 'step-1', status: 'completed' });
      await expect(bypassStep('step-1', 'admin-1', 'reason', 'comment', 'OUTCOME', mockDeps)).rejects.toThrowError('Cannot bypass a step instance that is not active');
    });

    it('successfully bypasses step and delegates to resolveNextStep', async () => {
      mockDeps.workflowRepository.getStepInstanceById.mockResolvedValue({ id: 'step-1', status: 'active', instanceId: 'inst-1' });
      mockDeps.workflowRepository.getInstanceById.mockResolvedValue({ id: 'inst-1', status: 'active' });
      mockDeps.workflowRepository.updateStepInstance.mockResolvedValue({ id: 'step-1', status: 'bypassed' });

      await bypassStep('step-1', 'admin-1', 'ADMIN_OVERRIDE', 'Justification', 'ADMIN_BYPASS_OUTCOME', mockDeps);

      expect(mockDeps.workflowRepository.updateStepInstance).toHaveBeenCalledWith('step-1', expect.objectContaining({
        status: 'bypassed',
        bypassedBy: 'admin-1',
  ...
  describe('migrateInstance', () => {
    it('VER-08: throws VALIDATION_FAILED if reason is empty', async () => {
      await expect(migrateInstance('inst-1', 'target-v2', 'admin-1', '  ', mockDeps)).rejects.toThrowError('migration reason must not be empty');
    });

    it('VER-09: throws INSTANCE_NOT_ACTIVE if instance is not active', async () => {
      mockDeps.workflowRepository.getInstanceById.mockResolvedValue({ id: 'inst-1', status: 'suspended' });
      await expect(migrateInstance('inst-1', 'target-v2', 'admin-1', 'reason', mockDeps)).rejects.toThrowError('Cannot migrate instance that is not active');
    });
    ...
    it('VER-10: throws STEP_KEY_NOT_FOUND_IN_TARGET_VERSION with full list of missing keys', async () => {
      mockDeps.workflowRepository.getInstanceById.mockResolvedValue({ id: 'inst-1', status: 'active', definitionId: 'def-1', definitionVersionId: 'v1' });
      
      // Target version has NO steps
      mockDeps.workflowRepository.getDefinitionVersionWithSteps.mockImplementation(async (id: string) => {
        if (id === 'target-v2') return { version: { publishedAt: new Date(), definitionId: 'def-1' }, steps: [], transitionRules: [] };
        if (id === 'v1') return { version: { definitionId: 'def-1' }, steps: [{ id: 'old-step-1', stepKey: 'missing_key_1' }, { id: 'old-step-2', stepKey: 'missing_key_2' }] };
        return null;
      });
      ...
      const err = await migrateInstance('inst-1', 'target-v2', 'admin-1', 'reason', mockDeps).catch(e => e);
    ...
    it('VER-04, VER-13: successfully migrates and consumes grant atomically', async () => {
      ...
      mockDeps.workflowRepository.getDefinitionVersionWithSteps.mockImplementation(async (id: string) => {
        if (id === 'target-v2') return { version: { publishedAt: new Date(), definitionId: 'def-1' }, steps: [{ id: 'new-step-1', stepKey: 'key_1' }], transitionRules: [] };
        if (id === 'v1') return { version: { definitionId: 'def-1' }, steps: [{ id: 'old-step-1', stepKey: 'key_1' }] };
        return null;
      });
    ...
    it('VER-14: sets instance to stuck if transition rules reference stale steps', async () => {
      ...
      expect(mockDeps.workflowRepository.updateInstanceStatus).toHaveBeenCalledWith('inst-1', 'stuck', undefined, mockTrx);
      expect(mockDeps.workflowRepository.createWorkflowEvent).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('reverseMigration', () => {
    it('VER-11: reverses within 24 hours without new grant', async () => {
      ...
      mockDeps.workflowRepository.getInstanceById.mockResolvedValue({ id: 'inst-1', status: 'active', definitionId: 'def-1', definitionVersionId: 'target-v2' });
      
      mockDeps.workflowRepository.getDefinitionVersionWithSteps.mockImplementation(async (id: string) => {
        if (id === 'v1') return { version: { definitionId: 'def-1' }, steps: [{ id: 'old-step-1', stepKey: 'key_1' }], transitionRules: [] };
        if (id === 'target-v2') return { version: { definitionId: 'def-1' }, steps: [{ id: 'new-step-1', stepKey: 'key_1' }] };
        return null;
      });
