...
describe('Transition Resolution (RES)', () => {
  describe('RES-V: evaluateTransitionRules', () => {
    it('RES-V-01: exact outcome_filter match wins over wildcard (null) at same priority ordering', () => {
      const rules = [
        { id: 'r1', fromStepId: 's1', toStepId: 'step-end', outcomeFilter: null, priority: 1, conditionExpression: null },
        { id: 'r2', fromStepId: 's1', toStepId: 'step-approved', outcomeFilter: 'APPROVED', priority: 10, conditionExpression: null },
      ] as any[];
    ...
    it('RES-V-02: specific outcome_filter at lower priority number beats wildcard at higher priority number', () => {
      const rules = [
        { id: 'r1', fromStepId: 's1', toStepId: 'step-specific', outcomeFilter: 'APPROVED', priority: 5, conditionExpression: null },
        { id: 'r2', fromStepId: 's1', toStepId: 'step-wildcard', outcomeFilter: null, priority: 10, conditionExpression: null },
      ] as any[];
      const result = evaluateTransitionRules(rules, 'APPROVED', {});
    ...
    it('RES-V-03: non-matching outcome_filter is excluded', () => {
      const rules = [
        { id: 'r1', fromStepId: 's1', toStepId: 'step-rejected', outcomeFilter: 'REJECTED', priority: 1, conditionExpression: null },
      ] as any[];
      const result = evaluateTransitionRules(rules, 'APPROVED', {});
    ...
    it('RES-V-05: conditionExpression evaluated with jsonLogic against context', () => {
      const rules = [
        {
          id: 'r1', fromStepId: 's1', toStepId: 'step-wealthy',
          outcomeFilter: null, priority: 1,
          conditionExpression: { '==' : [{ 'var': 'is_wealthy' }, true] },
        },
        {
          id: 'r2', fromStepId: 's1', toStepId: 'step-default',
          outcomeFilter: null, priority: 10,
          conditionExpression: null,
        },
      ] as any[];
      expect(evaluateTransitionRules(rules, 'APPROVED', { is_wealthy: true })).toBe('step-wealthy');
      expect(evaluateTransitionRules(rules, 'APPROVED', { is_wealthy: false })).toBe('step-default');
    });
  ...
  describe('RES-I: Scheduler-only outcomes via submitStepApproval', () => {
    ...
    it('RES-I10: LAPSED submitted by user → FORBIDDEN with LAPSED_IS_SCHEDULER_ONLY', async () => {
      ...
      try {
        await submitStepApproval(mockInstance, mockStepInstance, 'user-mayor', 'user', 'LAPSED', null, mockDeps);
    ...
    it('RES-I11: DEEMED_APPROVED submitted by user → FORBIDDEN with DEEMED_APPROVED_IS_SCHEDULER_ONLY', async () => {
      ...
      try {
        await submitStepApproval(mockInstance, mockStepInstance, 'user-mayor', 'user', 'DEEMED_APPROVED', null, mockDeps);
        expect.unreachable('Should have thrown');
      } catch (e: any) {
        expect(e.message).toContain('FORBIDDEN');
    ...
    it('RES-I12: OVERRIDE_SUCCEEDED with voteCount < 8 throws insufficient votes', async () => {
      ...
      await expect(
        submitStepApproval(mockInstance, mockStepInstance, 'user-mayor', 'user', 'OVERRIDE_SUCCEEDED', null, mockDeps)
    ...
    it('RES-I13: OVERRIDE_FAILED with voteCount >= 8 throws override failed but count is >= 8', async () => {
      ...
      await expect(
        submitStepApproval(mockInstance, mockStepInstance, 'user-mayor', 'user', 'OVERRIDE_FAILED', null, mockDeps)
