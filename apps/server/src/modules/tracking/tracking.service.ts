...
export function createTrackingService(repository: TrackingRepository): TrackingPublicAPI {
  return {
    async getTrackingRecordForDocument(documentId: string): Promise<TrackingRecordSummary | null> {
      return repository.findTrackingRecordByDocumentId(documentId);
    },

    async getRoutingHistory(documentId: string, actorId: string): Promise<RoutingEntry[]> {
