...
const CHAIN_VALIDATE_ROLES = ['sys_admin', 'auditor'];
...
function enforceRole(roles, allowed, message) {
 ...
 */
export function createAuditTrpcRouter(auditService) {
    return router({
        ...
        queryEvents: protectedProcedure
            ...
            const ALLOWED_ROLES = ['sys_admin', 'auditor'];
            const hasRole = ctx.auth.roles.some((r) => ALLOWED_ROLES.includes(r));
            if (!hasRole) {
                throw new TRPCError({
                    code: 'FORBIDDEN',
            ...
            const service = auditService ?? ctx.req.server.auditService;
        ...
        listOwnOfficeDocumentActions: protectedProcedure
            .input(paginationInput.merge(dateRangeInput).extend({
        ...
        validateChainIntegrity: protectedProcedure
            ...
            const env = ctx.req.server.auditEnv;
            const hmacSecret = env?.AUDIT_HMAC_SECRET ?? ctx.req.server.config?.AUDIT_HMAC_SECRET;
            ...
            if (input.fromEventId) {
                ...
                rows = await repo.db
                    .select()
                    .from(auditEvents)
                    .where(and(eq(auditEvents.cityId, ctx.auth.cityId), gte(auditEvents.sequenceNumber, startSeq)))
