...
function verifyJwt(token) {
    ...
    const algorithm = env.AUTH_JWT_ALGORITHM;
    ...
    return {
        ...
        rid: Array.isArray(decoded['rid']) ? decoded['rid'] : [],
        perm: Array.isArray(decoded['perm']) ? decoded['perm'] : [],
        cid: Array.isArray(decoded['cid']) ? decoded['cid'] : [],
        dg: (decoded['dg'] ?? null),
 ...
 */
export async function verifyAccessToken(request, reply) {
    ...
    const permissions = await this.iamRepository.findPermissionsByRoleIds(activeRoles.map((ra) => ra.roleId));
 ...
 */
async function loadDelegationContext(request, _reply) {
    ...
    auth.effectiveOfficeIds = [auth.officeId, ...grant.scope.officeIds].filter((id) => id !== null);
 ...
 */
async function setDatabaseSessionVars(request, reply) {
    const auth = request.auth;
    if (!auth) {
    ...
    const roleTier = auth.isItAdmin
        ...
        : auth.roles.includes('auditor')
            ? 'SECURITY_ADMIN'
            : 'STANDARD';
    const bypassOfficeIsolation = auth.roles.includes('sp_secretary') ||
        auth.roles.includes('sp_presiding_officer') ||
        auth.roles.includes('mayor') || // Cross-office read for mayor
    ...
    let resolveTx;
    let rejectTx;
    const txOpen = new Promise((resolve, reject) => {
        resolveTx = resolve;
        rejectTx = reject;
    });
    let resolveGucs;
    let rejectGucs;
    const gucsReady = new Promise((resolve, reject) => {
        resolveGucs = resolve;
        rejectGucs = reject;
    });
 ...
 */
async function updateLastActivity(request, _reply) {
    const auth = request.auth;
    if (!auth)
        return; // Guard — Hook 1 already enforced auth presence.
 ...
 */
export const authMiddlewarePlugin = fp(async function authMiddlewarePluginFn(fastify) {
