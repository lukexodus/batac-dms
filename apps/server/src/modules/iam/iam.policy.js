 ...
 */
const PLATFORM_ADMIN_ALLOWED_ACTIONS = new Set([
    'manage_roles',
    'manage_workflow_def',
 ...
 * Source: I1 §2 Gate 2.
 */
const IT_ADMIN_ISOLATED_RESOURCE_TYPES = new Set([
    'document_version',
    'document_attachment',
]);
/**
 ...
 * Source: I1 §2 Gate 2.
 */
const IT_ADMIN_ISOLATED_ACTIONS = new Set([
    'read',
    'download',
    'export',
    'bulk_export',
 ...
 * Source: I1 §2 Gate 4.
 */
const CLASSIFIED_LEVELS = new Set(['confidential', 'restricted']);
/**
 * Actions still permitted on soft-deleted resources (Gate 5).
 * Source: I1 §2 Gate 5.
 */
const SOFT_DELETE_ALLOWED_ACTIONS = new Set([
    'read',
    'read_metadata',
    'view_audit_trail',
]);
...
export class PolicyGuard {
     ...
     */
    checkGates(subject, resource, action) {
     ...
     */
    async checkClassificationGateAsync(subject, resource) {
        if (typeof resource.classificationLevel !== 'string' ||
            !CLASSIFIED_LEVELS.has(resource.classificationLevel)) {
...
export class PolicyEvaluator {
     ...
     */
    registerResourceHandler(resourceType, handler) {
 ...
 */
function sessionResourceHandler(subject, resource, action, context) {
    switch (action) {
        case 'read_own':
 ...
 */
function delegationGrantResourceHandler(subject, resource, action, context) {
    switch (action) {
        case 'create':
