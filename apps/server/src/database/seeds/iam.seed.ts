...
const ROLE_DEFINITIONS = [
  {
    ...
    description: 'Infrastructure; no document content access',
  },
  {
    code: 'plat_admin',
    name: 'Platform Administrator',
    typeCode: 'platform_admin',
    isSystemRole: true,
    isPlatformAdmin: true,
    description: 'Configuration; no document processing',
  },
  {
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Archiving, retention, disposition',
  },
  {
    code: 'dept_encoder',
    name: 'Department Encoder',
    typeCode: 'document_processor',
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Create and submit documents for their office',
  },
  {
    code: 'dept_approver',
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Approve documents at their office level',
  },
  {
    code: 'sp_secretary',
    name: 'SP Secretary',
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Full SP legislative document lifecycle',
  },
  {
    code: 'sp_member',
    name: 'SP Member',
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Review, comment, vote on legislative documents',
  },
  {
    code: 'sp_presiding_officer',
    name: 'SP Presiding Officer',
    typeCode: 'document_processor',
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Certify SP legislative output',
  },
  {
    code: 'mayor',
    name: 'Mayor',
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Highest executive approval authority',
  },
  {
    code: 'brgy_encoder',
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Submit documents on behalf of a barangay',
  },
  {
    code: 'brgy_captain',
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Approve and sign barangay-originated documents',
  },
  {
    code: 'auditor',
    name: 'Auditor',
    ...
    isSystemRole: false,
    isPlatformAdmin: false,
    description: 'Read-only: finalized documents and audit logs',
  },
  {
    code: 'citizen',
    name: 'Citizen',
...
interface PermRule {
  ...
  roles: Record<
    string,
    { decision: 'allow' | 'deny' | 'conditional'; conditionReference?: string }
  >;
}
...
const PERMISSION_RULES: PermRule[] = [
  ...
  buildRule('iam_user', 'read', {
    sys_admin: 'allow',
    plat_admin: 'allow',
    records_officer: 'allow',
    sp_secretary: 'allow',
    sp_presiding_officer: 'allow',
    mayor: 'allow',
    auditor: 'allow',
  ...
  buildRule('session', 'register_citizen', { citizen: 'allow', sp_secretary: 'allow' }, [
    'sys_admin',
    'plat_admin',
    'records_officer',
    'dept_encoder',
  ...
  buildRule('workflow_step_instance', 'approve', {
    dept_approver: 'allow',
    sp_secretary: 'allow',
    mayor: 'allow',
    brgy_captain: 'allow',
  }),
  buildRule('workflow_step_instance', 'reject', {
    dept_approver: 'allow',
    sp_secretary: 'allow',
    mayor: 'allow',
    brgy_captain: 'allow',
  }),
  buildRule('workflow_step_instance', 'return', {
    dept_approver: 'allow',
    sp_secretary: 'allow',
    mayor: 'allow',
    brgy_captain: 'allow',
  ...
  buildRule('tracking_record', 'scan_qr_public', { citizen: 'allow' }, [
    'sys_admin',
    'plat_admin',
    'records_officer',
    'dept_encoder',
  ...
  buildRule('session_attendance', 'manage', {
    sp_secretary: 'allow',
    sp_member: 'allow',
    sp_presiding_officer: 'allow',
    mayor: 'allow',
  ...
  buildRule('order_of_business', 'manage', {
    sp_secretary: 'allow',
    sp_member: 'allow',
    sp_presiding_officer: 'allow',
    mayor: 'allow',
  ...
  buildRule('records', 'set_retention', {
    records_officer: 'allow',
    plat_admin: 'allow',
    sp_secretary: 'allow',
    auditor: 'allow',
  ...
  buildRule('document_request', 'release_copy', {
    sp_secretary: 'allow',
    sp_presiding_officer: 'allow',
    auditor: 'allow',
  }),
  ...
  buildRule('portal', 'view_public', { citizen: 'allow' }, [
    'sys_admin',
    'plat_admin',
    'records_officer',
    'dept_encoder',
  ...
  buildRule('report', 'view_panlalawigan_summary', {
    records_officer: 'allow',
    sp_secretary: 'allow',
    sp_member: 'allow',
    sp_presiding_officer: 'allow',
  ...
  await db.transaction(async (tx: any) => {
    ...
    for (const rule of PERMISSION_RULES) {
      ...
      const existing = await tx
        ...
        .from(permissions)
        .where(
          sql`${permissions.resource} = ${rule.resource} AND ${permissions.action} = ${rule.action}`,
        )
        .limit(1);
    ...
    if (!tableExists) {
      ...
      for (const grant of CROSS_OFFICE_GRANTS) {
        ...
        if (!roleId) {
          console.error(
            `[seed] Role code "${grant.roleCode}" not found in role map, cannot seed cross-office grant.`,
          );
          continue;
        }
    ...
    console.log(
      `Seeded: ${rolesSeeded} new roles (total ${Object.keys(roleMap).length} mapped), ${permsSeeded} new permissions (total ${Object.keys(permMap).length} cataloged), ${matrixEntriesSeeded} role_permission entries.`,
    );
  });
}
