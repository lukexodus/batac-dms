...
export function AllComponentsPage() {
  ...
  const sixRows = Array.from({ length: 6 }).map((_, i) => ({
    ...
    status: ['PENDING_MAYOR', 'PANLALAWIGAN_REVIEW', 'ARCHIVED'][i % 3] as React.ComponentProps<
      typeof StatusBadge
    >['state'],
  ...
  const docCards = Array.from({ length: 3 }).map((_, i) => ({
    ...
    documentState: 'APPROVED' as React.ComponentProps<
      typeof DocumentPreviewCard
  ...
  const workflowSteps = Array.from({ length: 7 }).map((_, i) => ({
    id: `step-${i}`,
    label: `Step ${i + 1}`,
    state: (i < 2 ? 'completed' : i === 2 ? 'active' : 'pending') as
      | 'completed'
      | 'active'
      | 'pending',
  }));
  ...
  const timelineEntries = Array.from({ length: 5 }).map((_, i) => ({
    id: `tl-${i}`,
    action: ['Transmitted', 'SignedByMayor', 'Vetoed', 'Archived', 'Transmitted'][
      i
    ] as React.ComponentProps<typeof RoutingHistoryTimeline>['entries'][number]['action'],
    actorName: 'John Doe',
  ...
  const oobNormal = {
    ...
    documentState: 'PANLALAWIGAN_REVIEW' as React.ComponentProps<
      typeof OrderOfBusinessRow
    >['item']['documentState'],
    committeeReferrals: [
      {
        id: 'c1',
        committeeName: 'Environment',
        status: 'PENDING' as React.ComponentProps<
          typeof CommitteeReferralBlock
        >['referrals'][number]['status'],
      },
    ],
  ...
  return (
    <AppShell
      ...
      sidebarContent={
        <Sidebar
          items={[
            { id: 'dev', label: 'Dev Components', icon: Search, href: '/dev/all-components' },
          ]}
          activeItemId="dev"
    ...
    >
      <div className="mx-auto max-w-7xl space-y-16 p-8">
        ...
        <section className="space-y-6">
          ...
          <div className="border-border-default overflow-hidden rounded-lg border bg-white">
            <table className="w-full text-left">
              <thead className="border-border-default border-b bg-neutral-50">
                <tr>
                  <th className="p-3 text-sm font-medium">Document</th>
                  <th className="p-3 text-sm font-medium">Status</th>
                  <th className="w-72 p-3 text-sm font-medium">SLA</th>
                </tr>
              </thead>
                  ...
                  <tr key={r.id}>
                    ...
                    <td className="p-3 align-middle">
                      <div className="max-w-[250px]">
                        <SLATimer deadlineAt={r.slaEnd} startedAt={r.slaStart} label="Review SLA" />
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {/* Section 2 */}
        <section className="border-border-default space-y-8 rounded-lg border bg-white p-6">
          ...
          <div>
            <h3 className="mb-4 text-sm font-semibold">Workflow Progress</h3>
            <WorkflowStepIndicator steps={workflowSteps} currentStepId="step-2" />
          </div>

          <div className="grid grid-cols-2 gap-8">
            <div>
              <h3 className="mb-4 text-sm font-semibold">Routing History</h3>
              <RoutingHistoryTimeline entries={timelineEntries} />
            </div>
            <div className="space-y-8">
              <div>
                <h3 className="mb-4 text-sm font-semibold">Scan Quality</h3>
                <div className="border-border-default flex items-center justify-between rounded-md border p-3">
                  <span className="font-mono text-sm">document_scan_001.pdf</span>
                  <ScanQualityIndicator score={85} showLabel={true} />
                </div>
              </div>
              <div>
                <h3 className="mb-4 text-sm font-semibold">QR Code</h3>
                <QRCodeDisplay
