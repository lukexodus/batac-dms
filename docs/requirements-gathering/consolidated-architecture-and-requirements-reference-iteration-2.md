...
**Status:** Post-Interview 2 (June 15) | Pre-Development Baseline
...
- All remaining unresolved items are in Part 14 with updated priority.
...
**Appropriation Ordinances** follow the same workflow as regular ordinances. Now included in Phase 1. `[CONFIRMED — Interview 2]`
"Operative in its entirety" = Panlalawigan outcome specific to Appropriation Ordinances; synonymous with "valid; can be implemented." `[CONFIRMED — Interview 2]`
Supplemental Appropriation Ordinances (allocate more to initial budget) follow the same flow.
...
- Publication: only ordinances **with penalty** require full newspaper publication (Ilocos Times). Full ordinance text published. Secretariat prepares. `[CONFIRMED — Interview 2]`
...
**Outcome handling — updated from Interview 2:**
...
When **VALID-IN-PART**:
- Response depends on the specific recommendation
- May add or change provisions
- Can refer to City Legal Office or concerned Committee
- Sometimes Secretariat handles correction without repassing `[CONFIRMED — Interview 2]`
...
- **VALID-IN-PART handling:** System marks the document VALID-IN-PART, attaches the Panlalawigan's response, places step in "Awaiting SP Secretariat Action." SP Secretary chooses: (1) Resolve as-is with mandatory comment; (2) Route to Legal Office; (3) Route back to committee for re-draft. 
- **RETURNED handling:** System flags high-priority, requires immediate City Legal Office and Mayor coordination. Manual review state.
...
**Current state:** Existing memos have QR codes attached. Existing system for this was not discussed in interviews. `[CONFIRMED — Interview 2]`
...
**Current state:** Existing memos have QR codes attached. `[CONFIRMED — Interview 2]`
...
**Routing rules from Interview 2:**
- Most letters go to the Vice Mayor for review/routing instructions
...
**Current state:** Existing letters have QR codes attached. `[CONFIRMED — Interview 2]`
...
### 4.14 Citizen Complaint — Transportation `[CONFIRMED; now Phase 1]`
...
**Process:** Committee renders report. SP Secretariat sends report to both complainant and respondent via Letters Sent. `[CONFIRMED]`

See also Part 4.15 for the three access modes that apply to both complaints and copy requests.
...
**Payment system:** Currently deferred (TBD). Phase 1 does not include active payment integration. `[CONFIRMED]`
...
| Franchise Ordinances | **Removed from all scope** — Franchise Section has separate jurisdiction and their own system. Platform: read-only external link to Franchise Section system. No CRUD. `[CONFIRMED — Interview 2]` |

---

### 4.17 Certification of Urgency `[NEW — Interview 2]`
...
| Number format | `[Unresolved — not yet confirmed]` |

**System integration:** When a Certification of Urgency is logged by Secretariat, the associated measure's workflow instance is updated: committee referral step is bypassed, workflow advances to Second Reading. The Certification document itself is logged with QR code and archived.
...
**Note on exact delimiter format:** Interviews show prefix and year-number combined (e.g., "SPR" then "2026-01"). Whether the assembled format is `SPR-2026-01`, `SPR 2026-01`, or another delimiter is `[Unresolved — requires confirmation]`. See Q-A01 in Part 14.
...
| Rule | Decision |
| ---- | -------- |
| Preliminary number format | `"Draft " + {series_prefix} + {YEAR} + "-" + {NN}` — assigned at secretariat logging |
...
| LMITS subscription | **Possibly renewed** as of Interview 2 — Interview 2 lists "Renewed" as a bullet. Contradicts Interview 1 finding ("Ended — system is down"). `[Requires verification]` |
| LMITS accessible data | Titles of resolutions; keyword search; title and status `[CONFIRMED — Interview 2]` |
| LMITS migration scope | Titles and status confirmed available. Additional fields and migration format require confirmation. |
| Current Records Officer tooling | MS Word with keyword search for records |
| Physical records | Not yet in any digital system |
| SP website sp.batac.gov.ph | Still referenced as template download source as of Interview 2 |
| Existing QR codes | Confirmed: existing memos and letters currently have QR codes attached. System used for this was not discussed. `[CONFIRMED — Interview 2]` |
...
- Does not block on individual committee absentees
- Does not log committee absentees
- Completes when the unified committee report is submitted and accepted by the SP Secretary
- If the step is manually advanced by the SP Secretary (overriding a missing report), this must be audit-logged with a mandatory comment
...
The `workflow` module's step type for committee referral must support a list of assigned committee roles. The data model already reserves `parallel_split` and `parallel_join` step types for Phase 2. Phase 1 requires a `multi_referral` step type that does not track committee absentees and completes on unified report submission. This is a schema decision to make before the first workflow module migration.
...
| multi_referral | Assigns to multiple committees simultaneously; completes when unified report submitted and accepted | Phase 1 |
...
**Certified Urgent path — Phase 1 (not Phase 1B) `[CONFIRMED — Interview 2]`:**
...
- Upon logging: the associated measure's workflow instance bypasses the committee referral step and advances directly to Second Reading
...
**Cover sheet / QR code:** Separate cover page (not overlaid), auto-generated on print. Contains: tracking number, QR code, document type, author, date, approvers, retention schedule. The "cover page before printing" referenced in Interview 2 may be this cover sheet — requires confirmation. See Q-B02 in Part 14.
...
**OCR:** Full-text search across all documents is desired. OCR processing policy is `[Unresolved]` — see Q-C01 in Part 14.
...
| Decision | Value |
| -------- | ----- |
| Preliminary number | Assigned at secretariat logging. Uses "Draft" prefix. Can change before finalization. `[CONFIRMED — Interview 2]` |
...
**Administration transition interaction:** In-flight documents at administration change — the person who presided when the document was last active still signs/approves. Those designated can sign. In practice, new resolutions/ordinances are rarely submitted when an election is nearing. `[CONFIRMED — Interview 2]`
...
**Included:** IAM, Organization module, Document Core (SP Resolution/Ordinance/Appropriation Ordinance series; two-stage numbering), Workflow Engine (linear + branching + Certified Urgent path + multi-committee referral), SP Resolution workflow (full; 2 readings), SP Ordinance workflow (full; 3 readings), Appropriation Ordinance workflow (same as Ordinance), Certification of Urgency document type, Transmittal Letter generation, DTS (QR at logging + routing history + scan-to-lookup), Session attendance tracking, Order of Business view, Secretariat decision logging (Approve/Reject/Amended), In-app notifications, SP Secretary dashboard, Mayor dashboard, Audit log, Citizen Complaint module (three access modes), Public portal (Phase 1 subset: track by number + published documents with first-page preview), Infrastructure.
...
**Questions resolved by Interview 2 (removed from active list):**
...
- Q-05 Certified Urgent Authorization — PARTIALLY RESOLVED: Mayor issues formal written document; Secretariat logs; frequent. Remaining: number format, attachment rules — see Q-B01 below.
- Q-11 Publication Requirements — PARTIALLY RESOLVED: ordinances with penalty = full newspaper publication; resolutions = title + first page public only. Remaining: arrangement and payment details — see Q-C04 below.

---

For all the following questions, the answers of developers are wrapped with square brackets (`[`, `]`).

### 14.1 Priority 1 — Critical (blocks schema or core workflow engine)

#### Q-A01 — Number Format Delimiter Convention

Prefixes confirmed (SPR, SPS, MO, MI, NOSP, NCH, D). Exact delimiter between prefix and year-number not confirmed.

1. Is it `SPR-2026-01`, [`SPR 2026-01`], `SPR2026-01`, or another pattern?
2. Same for SPS, MO, MI, NOSP, D. [Yes.]
3. For the "Draft" prefix on resolutions/ordinances: is the format [`Draft 7SP 2026-02` (with a space)] or `Draft-7SP-2026-02` or another form?

> **Developer note:** The delimiter determines the format string stored in `number_series.format`. It affects QR code labels, document cover sheets, and all display fields. Must be confirmed before numbering module implementation.

---

#### Q-A02 — Multi-Committee Referral: All Reports Required Before Proceeding?

Interview 2 confirmed: absent committees don't stop hearings; unified report produced.

1. Must [**all** assigned committees sign/contribute to the unified report] before the workflow step completes, or can a single primary committee produce the report? [They all sign. But in the Order of Business document, the committees absent or the committees that have not yet made their report are marked red.]
2. If one committee never submits a contribution, can the SP Secretary manually advance the step? [Yes. But as mentioned, in the Order of Business, they are marked red.]
3. Is there a defined deadline for committee reports tied to the Thursday Order of Business cutoff? [If they haven't finished their report before Thursday finishes, then the second reading for that Resolution/Ordinance won't proceed at the immediate Tuesday next week , but only proceed at Tuesday after the week they have submitted it.]

> **Developer note:** Determines whether `multi_referral` step completion requires acknowledgment from all assigned committees or only one report submission. Affects step-completion trigger logic in the workflow engine.

---

### 14.2 Priority 2 — High (needed before Phase 1 development begins)

#### Q-B01 — Certification of Urgency: Document Details

Partially resolved: Mayor issues formal written document; Secretariat logs; frequent. Remaining:

1. Is there a specific numbering format for the Certification of Urgency document itself? [No. It is always associated with the Document (ex. Resolution or Ordinance) concerned.]
2. Is the Certification attached to the specific legislative measure in the system, or filed as a standalone document? [Attached.]
3. Is the Certification specific to a single measure, or can one Certification cover multiple measures in the same session? [Can cover multiple measures.]

---

#### Q-B02 — Cover Page Format Before Printing

Interview 2 notes reference a "cover page" that appears before printing.

1. What information appears on the cover page? [QR Code, Tracking Number, Series Number]
2. Is this the same as the QR cover sheet described in Part 11.4 (tracking number, QR code, metadata), or a separate document? [Yes. It is the same. But override: only shows QR Code, Tracking Number, Series Number. It does not have to be a whole paper size. It only takes space as it needs. To save paper, when printing cover pages, allow multiple cover pages (horizontal rectangle) to be included in one paper. Configurable in the system.]
3. Is it [auto-generated] by the system from document metadata, or manually prepared?

[Cover page and the QR cover sheet is the same.]

> **Developer note:** If the cover page is the same as the QR cover sheet, no additional work is needed. If it is a separate document, its fields and generation rules need to be captured before the PDF generation module is implemented.

---

#### Q-B03 — Originating Office in SP Context

Interview 2 notes flag: "Originating office only concerned is secretariat."

1. For documents created within the SP workflow (resolutions, ordinances), is the "Originating Office" always the SP Secretariat regardless of which Councilor drafted the document? [Yes.]
2. For letters received from external offices, what is recorded as the originating office — the external sender or the SP Secretariat? [External Sender]

> **Developer note:** Affects the `originating_office_id` field on the `documents` table and office hierarchy displays.

---

#### Q-B04 — Complaint Routing and Resolution Workflow

Complaint added to Phase 1. Full routing details needed.

1. After a complaint is logged, what is the routing path — to the committee directly, or first to the Vice Mayor? [Secretariat decides.]
2. Who resolves a complaint — the committee issues a report, and then what action is taken? [The committee issues a report. Secretariat logs; sends the report to the complainant (through the channel discussed below); and mark as resolved.]
3. Are there specific outcome states for complaints (resolved, dismissed, pending hearing)? [Yes. Follow that three. Add status for when the concerned (VM and/or Committee) as already seen/received the complaint.]
4. Does the respondent (e.g., tricycle operator) receive a formal written notice? Via what channel? [Yes given a formal written notice. When respondent provides email, the notification and the formal written notice is sent. When only contact number is provided, only notification is sent and that the respondent can now claim the formal written notice from the LGU.]
5. Can a complaint be filed against a non-tricycle subject (other LGU-related complaints), or is the Phase 1 Complaint scope specific to transportation? [Complaint can be filed against a non-tricycle subject.]

---

### 14.3 Priority 3 — Medium (needed before Phase 1 deployment)

#### Q-C01 — OCR Processing Policy

Full-text search desired; all documents should be OCR-processed.

1. Should [OCR run automatically on upload], or as a manual step triggered by the Records Officer? [Have a mechanism for detecting scan quality and always show this to the user so that that user can decide if he/she does manual scan.] 
2. Is OCR required for historical records migrated from LMITS, or only newly uploaded documents? [Yes. We do OCR on migration.]
3. How should the system handle OCR failures or poor-quality scan output? [Answered on number 1.]

---

#### Q-C02 — Panlalawigan 30-Day Timer: Operational Confirmation

1. When the Panlalawigan acts within 30 days, does the SP Secretariat receive a formal written notification (Panlalawigan resolution)? [Yes.]
2. When 30 days lapse with no action, does the SP Secretary currently record this manually, or does it sometimes go unnoticed? [Yes this is recorded. Marked as lapsed.]
3. Historical examples of VALID-IN-PART returns: did SP accept as-is, consult Legal, or re-vote? [Secretariat decides. Could: change the document by themselves, repass (goes back to Drafting) the document, consult Legal, etc.]

---

#### Q-C03 — LMITS Migration: Scope and Format

LMITS accessible data confirmed: titles, status, keyword search. Subscription possibly renewed.

1. Beyond title and status, what other fields exist in LMITS that need migration? [Migrate what can be migrated.]
2. In what format does the LMITS data currently exist — database export, spreadsheets, combination? [Not identified yet. Migration happens at later phases.]
3. Who currently has access to LMITS data for extraction (CPDO managed it)? [CPDO]
4. Is the LMITS subscription confirmed renewed? If so, is direct data export possible? [Not determined yet.]

---

#### Q-C04 — Newspaper Publication: Arrangement Details

Partially resolved: ordinances with penalty = full newspaper publication; resolutions = title + first page only (full copy by paid request).

1. Who arranges placement with Ilocos Times — SP Secretariat or Mayor's Office? [SP Secretariat.]
2. Is the publication date a mandatory tracked field in SP records? [Yes.]
3. For ordinances without penalties: any publication or public notice requirement? [No publication. But shown on the public portal.]

---

#### Q-C05 — Hearing Schedule: System Input Rules

Partially resolved: committee schedules second reading; secretariat schedules first reading; secretariat logs hearing schedules when received. [Session for hearings for resolutions and ordinances are always on Tuesday. But for the hearings by the committees themselves and other concerned people/departments, they decide themselves. They will all make one final report. They have deadline which is Thursday. If they aren't able to finish it by Thursday, then the Second reading is delayed. Certified Urgent Resolutions and Ordinances skips committee review and report.]

1. In the system, who inputs the committee hearing date — [Secretariat staff entering what the committee communicates], or can committee representatives input directly?
2. Can a committee referral step begin without a scheduled date ("assigned; date TBD")? [Yes.]

---

#### Q-C06 — RETURNED Ordinance: Formal Procedure When Already Implemented

Implementation is usually stopped when Panlalawigan returns. Remaining:

1. Is there a formal legal process to challenge the return — City Legal, Mayor, or SP vote? [No.]
2. Has this scenario (RETURNED after implementation started) occurred in Batac? If so, what was the outcome? [If it happens. It will be repassed (goes back to drafting).] 

---

#### Q-C07 — sp.batac.gov.ph Data Migration

1. Can the data on sp.batac.gov.ph (titles, first-page previews) be regenerated from LMITS source data and uploaded documents, or does it contain unique data not available elsewhere? [Migration will happen some time after the whole system is already developed and has been used for a significant amount of time. Defer this decision.]
2. Is formal retirement of sp.batac.gov.ph required, or does it simply remain offline until the new system launches? [Subscription has been renewed. Usage of it will continue indefinitely for now. The batac-dms is a new system primarily for internal use but has a public portal similar to the sp.batac.gov.ph.]

---

### 14.4 Priority 4 — Low (Phase 1B or Phase 2 clarification)

#### Q-D01 — VM Letter Review: Formal Categorical Rules

Partially resolved: letters/memos addressed to SP employees typically bypass VM; most letters go to VM.

1. Are there formally defined categories of incoming letters that the VP always reviews vs. those that go directly to Secretariat action?
2. If informal: what kinds of letters typically trigger VM review vs. direct action? 

[Secretariat decides. Almost all require VM (Vice Mayor) reviews because it is addressed to him or to his office. But if secretariat sees that the document does not require VM review then does not route to VM.]

---

#### Q-D02 — Administration Change: Formal Transition Procedure

Partially resolved: presiding officer at time of last action still signs/approves in-flight documents; designated persons can sign.

1. Is there a formal transition procedure (handover meeting, pending document list) the SP Secretariat currently performs? [No.]
2. When a new Mayor takes office, do [in-flight documents requiring the prior Mayor's signature automatically wait for the new Mayor], or is manual reassignment required?

---

#### Q-D03 — VALID-IN-PART: SP Re-Voting Practice

1. When the Panlalawigan marks a document VALID-IN-PART, does the SP re-vote on only the invalid provisions, or must the entire document be re-passed? 
2. Has VALID-IN-PART occurred for Batac documents historically? If so, what did the SP do?

[The action to be taken depends on the recommendation provided. For example, if there is a provision that needs to be added or amended, the matter may be referred to the City Legal Office for appropriate review. The matter may also be referred to the concerned Committee for further evaluation and recommendation. In some instances, the Secretariat implements the necessary revisions directly without requiring the matter to be re-passed.]

---

#### Q-D04 — Payment and Fee Structure for Copy Requests

1. What is the current fee structure (Ordinance No. 3SP 2014-05 referenced on form)?
2. Who collects payment — Secretariat or Treasury?
3. How is payment confirmation communicated to the Secretariat before the copy is released?

[To be implemented at later stages. Later still than the phases mentioned.]

---

#### Q-D05 — Franchise External System: Link Details

1. What is the URL of the Franchise Section's external system?
2. Is it a web page link or an API?
3. What information is visible via the link (franchise list, renewal status, operator details)?

[To be decided later.]

---

#### Q-D06 — Current QR System for Letters and Memos

Interview 2 confirmed existing memos and letters have QR codes. Existing system not discussed.

1. What system generates and tracks the existing QR codes on letters and memos?
2. Will existing QR codes need to be migrated or replaced when Phase 1B launches?
3. Is there a database behind the existing QR codes, or are they print-only labels?

[Assume this doesn't exist for now. Implement this system in batac-dms itself.]
