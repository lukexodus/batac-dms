...
1. Must **all** assigned committees sign/contribute to the unified report before the workflow step completes, or can a single primary committee produce the report?
2. If one committee never submits a contribution, can the SP Secretary manually advance the step?
3. Is there a defined deadline for committee reports tied to the Thursday Order of Business cutoff?
...
1. Is there a specific numbering format for the Certification of Urgency document itself?
2. Is the Certification attached to the specific legislative measure in the system, or filed as a standalone document?
3. Is the Certification specific to a single measure, or can one Certification cover multiple measures in the same session?
...
1. What information appears on the cover page?
2. Is this the same as the QR cover sheet described in Part 11.4 (tracking number, QR code, metadata), or a separate document?
...
1. For documents created within the SP workflow (resolutions, ordinances), is the "Originating Office" always the SP Secretariat regardless of which Councilor drafted the document?
2. For letters received from external offices, what is recorded as the originating office — the external sender or the SP Secretariat?
...
1. After a complaint is logged, what is the routing path — to the committee directly, or first to the Vice Mayor?
2. Who resolves a complaint — the committee issues a report, and then what action is taken?
3. Are there specific outcome states for complaints (resolved, dismissed, pending hearing)?
4. Does the respondent (e.g., tricycle operator) receive a formal written notice? Via what channel?
5. Can a complaint be filed against a non-tricycle subject (other LGU-related complaints), or is the Phase 1 Complaint scope specific to transportation?
...
1. Should OCR run automatically on upload, or as a manual step triggered by the Records Officer?
2. Is OCR required for historical records migrated from LMITS, or only newly uploaded documents?
3. How should the system handle OCR failures or poor-quality scan output?

---

#### Q-C02 — Panlalawigan 30-Day Timer: Operational Confirmation

1. When the Panlalawigan acts within 30 days, does the SP Secretariat receive a formal written notification (Panlalawigan resolution)?
2. When 30 days lapse with no action, does the SP Secretary currently record this manually, or does it sometimes go unnoticed?
3. Historical examples of VALID-IN-PART returns: did SP accept as-is, consult Legal, or re-vote?
...
1. Beyond title and status, what other fields exist in LMITS that need migration?
2. In what format does the LMITS data currently exist — database export, spreadsheets, combination?
3. Who currently has access to LMITS data for extraction (CPDO managed it)?
4. Is the LMITS subscription confirmed renewed? If so, is direct data export possible?
...
1. Who arranges placement with Ilocos Times — SP Secretariat or Mayor's Office?
2. Is the publication date a mandatory tracked field in SP records?
3. For ordinances without penalties: any publication or public notice requirement?

---

#### Q-C05 — Hearing Schedule: System Input Rules

Partially resolved: committee schedules second reading; secretariat schedules first reading; secretariat logs hearing schedules when received.
...
2. Can a committee referral step begin without a scheduled date ("assigned; date TBD")?
...
1. Is there a formal legal process to challenge the return — City Legal, Mayor, or SP vote?
2. Has this scenario (RETURNED after implementation started) occurred in Batac? If so, what was the outcome?

---

#### Q-C07 — sp.batac.gov.ph Data Migration

1. Can the data on sp.batac.gov.ph (titles, first-page previews) be regenerated from LMITS source data and uploaded documents, or does it contain unique data not available elsewhere?
2. Is formal retirement of sp.batac.gov.ph required, or does it simply remain offline until the new system launches?
...
1. Is there a formal transition procedure (handover meeting, pending document list) the SP Secretariat currently performs?
...
_End of Remaining Open Questions. Total: 16 question clusters across 4 priority tiers. 9 questions resolved by Interview 2._
