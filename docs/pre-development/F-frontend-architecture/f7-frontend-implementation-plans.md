...
**Resolved — QRCodeDisplay composition.** *[Resolved by the A1 UI module pass, with human authorization — see `docs/pre-development/A-project-planning/a1-tasks/ui.md`, `TASK-UI-013`.]* The original Plans had `DocumentNumberBadge` as a Tier 3 dependency of `QRCodeDisplay`; F5 §4.3 lists `QRCodeDisplay` composition as `—` (no named primitives), leaving this document's table ambiguous between the two. J6 §3.11 — the canonical per-component engineering reference — resolves this with a complete implementation: the document number renders as plain styled text (`font-mono text-xs font-medium text-text-primary`) directly inside the component, with no Tier 1, Tier 2, or Tier 3 dependency at all. **Decision: `QRCodeDisplay` has no Tier 3 dependency**, per J6. Row 11 below reflects this.
