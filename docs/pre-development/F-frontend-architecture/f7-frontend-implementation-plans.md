...
**Open item — QRCodeDisplay composition.** The original Plans had `DocumentNumberBadge` as a Tier 3 dependency of `QRCodeDisplay`. F5 §4.3 lists `QRCodeDisplay` composition as `—` (no named primitives). This is not resolved in this document. The A1 UI module pass should flag it and the human should decide before the QRCodeDisplay task prompt is finalized. [Inference — QRCodeDisplay may display a document number string without composing the badge component; both implementations are plausible.]
...
|---|---|---|---|---|---|---|
| 1 | `PageHeader` | §6.1 | none | `Button (T2)` [corrected from F5: original said none] | none | title only; title + subtitle; title + subtitle + action button |
...
| 11 | `QRCodeDisplay` | §6.6 | none | none | `DocumentNumberBadge` [open item: F5 §4.3 lists no composition for this component; DocumentNumberBadge dependency is from original Plans and may be a rendering string rather than a component composition — flag during A1 UI module pass] | screen variant; print variant; `role="img"` and `aria-label` visible in debug annotation |
...
| 13 | `StatusBadge` | §6.3, §7 | `DocumentState`, `StatusMetaEntry` | none (CVA + pure Tailwind) — but STATUS_META from J6 required | none | all 23 states in a grid (DRAFT through MISSING_REPORT per F5 §4.3 DocumentState union); three states in a mock table row; `LAPSED` italic; `CANCELLED` line-through; `DEEMED_APPROVED` dashed border |
...
└── Tier 3 Group D — depends on Group B + C
    ...
    └── OrderOfBusinessRow   ← needs DocumentNumberBadge + StatusBadge + CommitteeReferralBlock
