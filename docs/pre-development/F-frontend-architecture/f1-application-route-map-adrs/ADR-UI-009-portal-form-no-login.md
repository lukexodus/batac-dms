# ADR-UI-009: No Authenticated Account Required for Portal Request/Complaint Forms
...
- `[Speculation]` Whether a citizen who registers an account _after_ a no-login submission can retroactively claim/link that submission to their account (so it then shows up under their authenticated status page) is not addressed by any source document or by this ADR. This is a follow-up design question for whoever builds the registration/linking flow.
