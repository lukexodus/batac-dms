...
- Domain types referenced in props interfaces (e.g., `DocumentState`) are defined in `packages/ui/src/types/domain.ts` and imported — never redefined locally. _[Corrected from `packages/shared` by the A1 UI module pass, following J6 §1 — see the package-overview note above for the circular-dependency rationale.]_
...
| `SLATimer`               | `@batac/ui/components/domain/SLATimer`               | §6.3, §8 Rule 6          | —                                                                            | Document in PENDING_MAYOR state             |
 ...
 */
type RoutingActionType = 'Transmitted' | 'Approved' | 'Returned' | 'Filed' | (string & {});
...
**`@source` scanning across package boundaries:** [Confirmed — `globals.css` lines 20–21] Tailwind v4 does not automatically scan files outside the package containing the CSS entry point. `globals.css` declares two explicit `@source` directives: `../components/**/*.{ts,tsx}` (this package's own components) and `../../../apps/web/src/**/*.{ts,tsx}` (the consuming web app). A class name used only in a file outside these two globs is not detected by Tailwind's scanner and produces no utility CSS, even if the class itself is otherwise valid. When a new app or package consumes `@batac/ui/styles/globals.css` — for example `apps/portal` in Phase 3 — a corresponding `@source` line for that app's source directory must be added to `globals.css`, or class names used only in that app will silently produce no styling. This is not currently enforced by any lint rule or test, so it depends on whoever wires up the new app remembering this step.
...
1. Locate this component's entry in the §4.3 overview table and copy its props interface verbatim as the starting point. If the interface imports a domain type (e.g., `DocumentState`), add `import type { DocumentState } from '@batac/ui/types/domain'` _[corrected from `@batac/shared` by the A1 UI module pass — see J6 §1 and the package-overview note in §1 above]_ — do not redefine domain types locally.
