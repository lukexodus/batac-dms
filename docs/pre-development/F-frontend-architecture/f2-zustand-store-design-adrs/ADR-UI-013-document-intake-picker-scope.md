# ADR-UI-013: CERTIFICATION_OF_URGENCY and DOCUMENT_REQUEST_FORM Excluded from `useDocumentIntakeStore` Step 1 Picker
...
`useDocumentIntakeStore` (F2 §8) drives a generic five-step "new document" intake flow: select type → core fields (title, classification, originating office) → type-specific metadata → file upload → review/submit. E3 Part 5 defines per-document-type metadata schemas for `SP_RESOLUTION`, `SP_ORDINANCE`, `APPROPRIATION_ORDINANCE`, `CITIZEN_COMPLAINT`, `DOCUMENT_REQUEST_FORM`, and `CERTIFICATION_OF_URGENCY` — all six therefore _could_ render a Step 3 form via the existing per-type dispatch pattern.
...
- **`DOCUMENT_REQUEST_FORM`** is created via the dedicated `useDocumentRequestIntakeStore` (F2 §11), which already models the in-person clerk-assisted flow with its own requester/purpose/payment/printable-PDF shape, distinct from the no-login citizen portal path (per ADR-UI-009).
...
- No change to `useModalStore` or `useDocumentRequestIntakeStore` is required by this ADR — both already model the correct flow. This ADR formalizes that the generic intake store is deliberately _not_ also wired to these two types.
