# ADR-WFL-005: `RecordType` Enum Value List
...
- **C1:** `records.record_type` (or equivalent column, once the `records` schema migration is written) can now be constrained with a `TEXT` column and a `CHECK` constraint using the six values above, rather than left as unconstrained `TEXT`. `[Corrected — previously said "CHECK constraint or native enum type"; the project's established convention (ADR-DB-002; consolidated reference Part 11.9's "Check constraints for state transitions" non-negotiable) is TEXT + CHECK, not native ENUM, specifically because CHECK constraints can be altered with ADD/DROP CONSTRAINT while native ENUM values require ALTER TYPE and cannot be removed without recreating the type]` C1 Part 15's open item for `RecordType` values is closed by this ADR for the *enum* question; the retention-period sub-question is tracked separately per "Deferred" above.
