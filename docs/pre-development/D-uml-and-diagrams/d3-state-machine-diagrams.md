...
**Document ID:** D3  
**Status:** Pre-Development Baseline — Pending Team Review  
**Platform:** Batac City LGU Platform  
**Last Updated:** June 2026  
**Audience:** Backend development team  
**Source Documents:** `consolidated-architecture-and-requirements-reference-iteration-3.md` (Parts 4, 5, 8, 11, 12); `b4-workflow-engine-specification.md` (Sections 2, 3, 4, 6)
...
**State set confirmed:** `Draft`, `Submitted`, `In-Workflow`, `Pending Approval`, `Completed`, `Released`, `Archived`, `Disposed`, `Cancelled`. Source: consolidated reference Part 11.4. `[CONFIRMED]`

**Terminal states:** `Disposed`, `Cancelled`. All others are non-terminal.
...
stateDiagram-v2
    ...
    InWorkflow --> PendingApproval : APPROVAL_STEP_REACHED
    PendingApproval --> InWorkflow : APPROVAL_RETURNED
    PendingApproval --> Completed : FINAL_APPROVAL_GRANTED
    ...
    PendingApproval --> Cancelled : DOCUMENT_CANCELLED
...
|`In-Workflow`|Formally logged by the Secretariat. QR code assigned. Preliminary Draft number assigned (for document types that use preliminary numbering). Workflow instance created and `Running`. Document is actively progressing through internal workflow steps (committee referral, readings, VP certification, transmittal). `[CONFIRMED — consolidated ref Part 11.6]`|No|
|`Pending Approval`|All internal workflow steps completed. Document awaiting external authority action. For SP Resolutions and Ordinances: covers Mayor's 10-day review window and, following Mayor action, the Sangguniang Panlalawigan's 30-day review. For other document types: covers the final signing or authorization step. Final series number already assigned at this point (assigned post-last-reading vote, before VP and Mayor sign). `[Inference: exact step mapping per document type]`|No|
|`Completed`|All required external approvals obtained or statutory lapses recorded. For SP Resolutions and Ordinances: Mayor has signed (or 10-day lapse per RA 7160 Section 47 recorded, or veto override succeeded), and Panlalawigan has returned VALID or DEEMED APPROVED (or 30-day lapse per RA 7160 Section 56(d)). `[Inference: that Panlalawigan outcome is a precondition for this state for Resolutions and Ordinances]`|No|
...
|`Cancelled`|Cancelled by an authorized actor. Document record permanently retained; no deletion. Numbers already assigned are flagged as cancelled with a gap record; they are never reused. Cancellation reason is mandatory. `[CONFIRMED — consolidated ref Part 11.7, Part 5.2]`|Yes|

### 1.3 Transition Table

All triggering event names and guard conditions are `[Inference]` derived from domain knowledge in the consolidated reference unless noted otherwise.

|From|To|Event|Guard Conditions|Notes|
|---|---|---|---|---|
|`Draft`|`Submitted`|`DOCUMENT_SUBMITTED`|Submitting actor is authorized to submit this document type; minimum required fields present|Physical submission to Secretariat is standard. Digital submission via system form is also valid (three access modes for complaints and document requests). `[CONFIRMED — Part 4.15]`|
|`Submitted`|`In-Workflow`|`WORKFLOW_INITIATED`|Secretariat staff completes formal intake action; a published and active workflow definition version exists for this document type|QR code assigned first (before any number). Preliminary Draft number assigned in same action for Resolutions and Ordinances. Workflow instance created and transitions immediately to `Running`. Source: consolidated ref Part 11.6, Part 5.2. `[CONFIRMED]`|
|`In-Workflow`|`Pending Approval`|`APPROVAL_STEP_REACHED`|All preceding internal workflow steps have reached a terminal state (Completed or Skipped); workflow engine advances to the external approval step|`[Inference]` For SP Resolutions and Ordinances: occurs after VP signs the certified copy and the Transmittal Letter to Mayor is generated. Final series number already exists at this point.|
|`Pending Approval`|`In-Workflow`|`APPROVAL_RETURNED`|External approver's action results in a workflow loop back to an internal processing step; the outcome maps to a defined In-Workflow routing target|`[Inference]` Known cases: (1) Mayor vetoes → veto override vote step activates (back to `In-Workflow`). (2) Panlalawigan RETURNED outcome → repassing or amendment process (back to `In-Workflow`). If override fails, the document transitions to `Cancelled` via workflow termination — not back to `Pending Approval`.|
|`Pending Approval`|`Completed`|`FINAL_APPROVAL_GRANTED`|All required external approvals obtained or statutory lapses recorded; no outstanding veto override or Panlalawigan repass pending|Legal bases: Mayor lapse = RA 7160 Section 47; Panlalawigan 30-day lapse = RA 7160 Section 56(d). `[CONFIRMED — consolidated ref Parts 4.1, 4.2, 4.3]`|
|`Completed`|`Released`|`DOCUMENT_RELEASED`|Secretariat docketing step complete; portal publication configured; for penalty ordinances: newspaper publication date is recorded (mandatory field)|SP Secretariat arranges newspaper placement for penalty ordinances (Ilocos Times). `[CONFIRMED — consolidated ref Part 4.2, Part 5.3]`|
|`Released`|`Archived`|`DOCUMENT_ARCHIVED`|Records Officer performs archiving action; retention schedule set and confirmed|SP Resolutions and Ordinances: permanent retention. `[CONFIRMED — consolidated ref Part 11.7]`|
|`Archived`|`Disposed`|`DISPOSITION_AUTHORIZED`|Retention period elapsed per schedule; document not under legal hold; Records Officer has recorded a mandatory non-empty comment; no automated disposal — explicit authorized action required|`[CONFIRMED — consolidated ref Part 11.7]` Not reachable for SP Resolutions and Ordinances (permanent retention). Disposition creates audit record; document data is not deleted.|
|`Draft`|`Cancelled`|`DOCUMENT_CANCELLED`|Actor has cancellation authority for this document type; mandatory cancellation reason recorded|No preliminary number assigned yet at this stage; no gap record needed.|
|`Submitted`|`Cancelled`|`DOCUMENT_CANCELLED`|Actor has cancellation authority; mandatory reason|No number assigned at Submitted; no gap record needed.|
|`In-Workflow`|`Cancelled`|`DOCUMENT_CANCELLED`|Actor has cancellation authority; mandatory reason|Preliminary Draft number gap logged if applicable. Associated workflow instance also receives `INSTANCE_CANCELLED`.|
|`Pending Approval`|`Cancelled`|`DOCUMENT_CANCELLED`|Actor has cancellation authority; mandatory reason|Covers administrative withdrawal and terminal veto override failure. Final series number already exists; gap logged with cancellation record.|
|`Completed`|`Cancelled`|`DOCUMENT_CANCELLED`|Elevated authorization required; mandatory detailed reason; dedicated audit entry|Unusual case. Final series number retained with cancellation flag.|
|`Released`|`Cancelled`|`DOCUMENT_CANCELLED`|Elevated authorization required; mandatory reason; portal publication must be retracted|Extremely rare.|
|`Archived`|`Cancelled`|`DOCUMENT_CANCELLED`|Records Officer authorization; highest authority required; mandatory detailed comment|Effectively a reclassification, not a deletion. Document remains fully retrievable.|

### 1.4 Notes

**No transition backwards through the main chain.** The lifecycle states are milestones. There is no `Released → Completed`, `Archived → Released`, or similar reversal. When a Panlalawigan RETURNED outcome requires a document to be repassed, the workflow engine handles this by routing back through internal steps (the document remains in `Pending Approval` or returns to `In-Workflow`) — not by reverting the lifecycle state to `Draft`. `[Inference — unresolved: see O-1 in Appendix C]`

**`Disposed` is permanently unreachable for SP Resolutions and Ordinances** under current retention policy (permanent retention confirmed in consolidated reference Part 11.7). The Records Officer validation at disposition time must enforce this. The state exists in the enum for other document types with finite retention schedules (internal memos, citizen correspondence, draft versions).

**QR code survives all lifecycle states.** The QR tracking number assigned during `WORKFLOW_INITIATED` persists through every subsequent state including `Cancelled` and `Archived`. It is immutable. Source: consolidated reference Part 11.6. `[CONFIRMED]`

**Numbering checkpoint within the lifecycle.** The preliminary Draft number is assigned during `WORKFLOW_INITIATED` (start of `In-Workflow`). The final series number is assigned after the last reading vote — which occurs _while_ the document is in `In-Workflow` state, before `APPROVAL_STEP_REACHED` fires. The document enters `Pending Approval` with the final series number already set. The "Draft" prefix is removed at this assignment. Source: consolidated reference Parts 5.1, 5.2, 11.5. `[CONFIRMED]`
...
|---|---|---|
|Document Lifecycle|`Disposed`, `Cancelled`|`Draft`, `Submitted`, `In-Workflow`, `Pending Approval`, `Completed`, `Released`, `Archived`|
...
**No document data is deleted in any terminal state.** Soft-delete (`deleted_at` timestamp) is the only permitted deletion mechanism, and it is not triggered by lifecycle state transitions. Physical document records are retained permanently in all states including `Disposed` and `Cancelled`. Source: consolidated reference Parts 11.7, 12 (Invariant 2). `[CONFIRMED]`
...
The following items are unresolved and block specific transition or schema implementations. None of the three diagrams above can be considered final until all items are resolved.

|#|Item|Blocks|Resolution Needed Before|
|---|---|---|---|
|O-1|Exact mapping of `Pending Approval` to specific workflow steps per document type. Specifically: is Panlalawigan review tracked within `Pending Approval` or within `In-Workflow`? `[Inference]`|Dashboard status display; public portal document status; document type workflow template authoring|Workflow definition authoring sprint|
|O-2|How is a Panlalawigan RETURNED → repass case modeled in the document lifecycle? Option A: the original document transitions `Pending Approval → In-Workflow → Draft` (lifecycle reversion). Option B: the original document is Cancelled; a new document revision starts its own lifecycle at `Draft`. Option C: other. Source: consolidated ref Part 4.3. `[Unverified — no stakeholder confirmation]`|Panlalawigan RETURNED workflow routing; document versioning design|Panlalawigan integration sprint|
...
These are the D3-authoritative enum definitions. B4 must align to these values. These stubs are for reference; the actual migration SQL is produced from the Drizzle schema.
...
_This document supersedes any conflicting state definitions in B4. Changes to enum values, state names, or transition semantics require an explicit revision entry dated and attributed, plus notification to all active implementers._