...
|---|---|---|---|
|`document.logged`|`document.created`|—|**[Discrepancy]** Trigger is identical (Secretariat logs a new document). This catalog uses `document.created` per B2 as the module boundary contract. Team must confirm.|
|`preliminary_number.assigned`|`document.number_assigned` with `numberType: 'preliminary'`|—|**[Discrepancy]** B3 context reference implies a separate event; B2 unifies both numbering events into one with a discriminator field. This catalog follows B2 (unified event). Team must confirm.|
|`final_number.assigned`|`document.number_assigned` with `numberType: 'final'`|—|**[Discrepancy]** Same as above row.|
|`certification_of_urgency.attached`|— (not listed in B2 emitted events for Documents)|`documents.certification_urgency.logged`|**[Discrepancy]** B2 omits this event from its Master Registry. This catalog uses B4's name. Note: the `documents.` prefix (plural) is inconsistent with all other Documents events (`document.` singular). Both the name and prefix require team ratification.|
...
> **Note `[Discrepancy]`:** B3 Context Reference §18 names this event `document.logged`. This catalog uses `document.created` per B2. Team must ratify.
...
> **Note `[Discrepancy]`:** B3 Context Reference §18 names these as two separate events: `preliminary_number.assigned` and `final_number.assigned`. B2 unifies them into one event with a `numberType` discriminator. This catalog follows B2. If the team prefers two separate events (enabling consumers to subscribe independently), this schema must be split and the event type strings updated accordingly.
...
#### 6.5 `documents.certification_urgency.logged`
...
> **Note `[Discrepancy — name]`:** B3 Context Reference §18 names this event `certification_of_urgency.attached`. B4 §6.1 names it `documents.certification_urgency.logged`. This catalog uses the B4 name.
> 
> **Note `[Discrepancy — prefix]`:** All other Documents module events use the `document.` prefix (singular). This event uses `documents.` (plural) per B4 §6.1. The inconsistency may be a B4 authoring error. Team must decide whether to normalize to `document.certification_urgency.logged`.
> 
> **Note `[Discrepancy — B2 omission]`:** B2's Master Event Registry does not list this event. It was identified in B4 §6.1 as produced by Documents and consumed by Workflow. The B2 Master Registry must be updated to include it.

```typescript
export const DocumentsCertificationUrgencyLoggedPayloadSchema = z.object({
...
export type DocumentsCertificationUrgencyLoggedPayload =
  z.infer<typeof DocumentsCertificationUrgencyLoggedPayloadSchema>;
```
...
export const WorkflowInstanceCreatedPayloadSchema = z.object({
  ...
  documentId:          z.string().uuid(),
  documentType:        z.string(), // [Inference] Full enum not confirmed for all workflow-capable document types
...
> **Note `[Inference — missing field]`:** B4 Appendix A lists only `instanceId`, `outcomeCode`, and `finalDocumentStatus`. The `documentId` is not in B4's payload for this event, but consuming modules (Records, Portal) need it to perform their work. Including `documentId` is strongly recommended; team must confirm. B2's equivalent event (`workflow.completed`) did not specify a payload — it is inferred from consumer needs.
...
export const WorkflowInstanceCompletedPayloadSchema = z.object({
  ...
  outcomeCode:         z.string(),                 // [Inference] exact outcome codes not specified in source
...
> **Note `[Discrepancy — field-level]`:** B4 Appendix A payload (`stepKey`, `assignedTo`) and B2 equivalent payload (`assigneeUserId`, `documentId`, `dueAt`) do not fully match. This schema merges both sources. Fields marked `[Inference]` are from B2's equivalent event and are required by consuming modules (e.g., Notifications needs `documentId` to compose the notification body; `dueAt` is the step due date for the notification).
...
export const WorkflowStepStartedPayloadSchema = z.object({
  ...
  documentId:     z.string().uuid().optional(),                           // [Inference] from B2 equivalent; needed by Notifications
  dueAt:          z.string().datetime({ offset: true }).nullable().optional(), // [Inference] from B2 equivalent
});
...
> **Note `[Discrepancy — audit annotation]`:** B4 Appendix A marks this event `(Audit for approval/multi_referral)` only, implying other step types may not be individually audited. B2's architectural law requires Audit subscription for all events. This catalog applies Audit to all `workflow.step.completed` emissions.
> 
> **Note `[Discrepancy — field-level]`:** B4 has `actorType` not in B2. B2 has `stepType` and `documentId` not in B4. Both sets are included. `stepType` is required by Tracking to compute action type. `documentId` is required by Tracking to look up the correct routing record. Fields from B2 marked `[Inference]` since B4 does not include them.

```typescript
export const WorkflowStepCompletedPayloadSchema = z.object({
  instanceId:     z.string().uuid(),
  stepInstanceId: z.string().uuid(),
  outcome:        z.string(),                    // [Inference] varies by stepType; exact enum not confirmed in source
  ...
  stepType:       WorkflowStepTypeSchema.optional(), // [Inference] from B2 equivalent; required by Tracking
  documentId:     z.string().uuid().optional(),      // [Inference] from B2 equivalent; required by Tracking
});
...
> **Note `[Discrepancy — field]`:** B4 Appendix A lists `cutoffTimestampCleared` as a payload field. B4 §6.2's EMIT block pseudocode does not include it. Field is included here as nullable per Appendix A; team should confirm its meaning and presence.
...
export const WorkflowMultiReferralSecondReadingEligiblePayloadSchema = z.object({
  ...
  cutoffTimestampCleared: z.string().datetime({ offset: true }).nullable(), // from B4 Appendix A; not in §6.2 EMIT block
});
...
A Certification of Urgency is a formal written document issued by the Mayor. When the Secretariat logs it, the `documents` module emits `documents.certification_urgency.logged` (§6.5), which the Workflow module consumes and processes. The Workflow module then emits one of the four events below depending on the state of each associated instance.
...
|**Consumers**|`notifications` `[Inference]` · `audit` `[Inference per B2 mandatory rule]`|
|**Source**|B4 Appendix A `[Unverified]`|

**Business Reason:** Provides an advance warning before a legally mandated SLA deadline is breached, giving assignees and supervisors time to accelerate processing. ARTA non-compliance carries administrative and civil liability consequences for LGU staff.

> **Note:** Notification consumer is `[Inference]`; B2 only specifies Notifications as consuming `workflow.escalated` (breach). Warning and critical consumers are not explicitly stated in either B2 or B4.
...
> **Note:** B2's `workflow.escalated` payload included `escalatedToUserIds: string[]` (the specific users notified). B4's `workflow.sla.breached` does not include this field. The Notifications module needs to know who to notify. [Inference] The Notifications module resolves the escalation targets from the workflow definition's escalation configuration at the time of notification, rather than receiving them in the event payload.
...
**Business Reason:** A second escalation threshold beyond initial breach for severely overdue documents. At 150% of the SLA window, the severity warrants a distinct notification to higher-level supervisors or department heads. Not present in B2; B4 defines the threshold.
...
export const WorkflowSlaCriticalPayloadSchema = z.object({
  ...
  stepInstanceId: z.string().uuid(),
  slaDeadline:    z.string().datetime({ offset: true }),
  percentElapsed: z.literal(150), // [Inference] implied by B4 trigger description "150% of SLA time elapsed"
});
...
|13|`documents.certification_urgency.logged`|`documents`|`workflow` · `audit` [Inf]|1|B4 §6.1|
...
|40|`workflow.sla.warning`|`workflow`|`notifications` [Inf] · `audit` [Inf]|1|B4 App A|
|41|`workflow.sla.breached`|`workflow`|`notifications` · `audit`|1|B4 App A|
|42|`workflow.sla.critical`|`workflow`|`notifications` [Inf] · `audit` [Inf]|1|B4 App A|
...
## §10 — Open Items — Team Decision Required Before Implementation

The following items cannot be resolved by the catalog authors. Each requires a team decision, recorded as an ADR or inline update to this document.

|#|Item|Blocking|Decision Needed|
|---|---|---|---|
|OI-1|Event name `document.created` vs `document.logged` (§0.1 row 1)|Yes|Pick one canonical name; update B3 and all consumers.|
|OI-2|`document.number_assigned` unified event vs two separate events `preliminary_number.assigned` / `final_number.assigned` (§0.1 rows 2–3)|Yes|If splitting: define two separate event type strings, two separate payload schemas, and separate consumer subscriptions.|
|OI-3|`documents.certification_urgency.logged` prefix — plural `documents.` vs singular `document.` (§6.5 notes)|Yes|Normalize the prefix; update B4 if changed.|
|OI-4|B4 `workflow.step.started` vs B2 `workflow.step_assigned` — which name is canonical? (§0.2, §7.11)|Yes|Ratify B4 name or revert to B2 name. Update all consumer registrations.|
|OI-5|`workflow.step.started` missing `documentId` and `dueAt` that exist in B2's equivalent (§7.11 notes)|Yes|Confirm whether `documentId` and `dueAt` are included in the payload. Notifications module needs both.|
|OI-6|`workflow.step.completed` missing `stepType` and `documentId` that exist in B2's equivalent (§7.12 notes)|Yes|Confirm whether both are included. Tracking module needs `stepType` to derive action type.|
|OI-7|`workflow.sla.breached` missing `escalatedToUserIds` that exists in B2's `workflow.escalated` (§7.28 notes)|Yes|Confirm whether Notifications resolves escalation targets from config at notification time, or whether the event must carry them.|
|OI-8|`workflow.instance.completed` missing `documentId` (§7.2 notes)|Yes|Confirm whether `documentId` should be in the payload for Records and Portal consumers.|
|OI-9|`workflow.multi_referral.second_reading_eligible` — presence of `cutoffTimestampCleared` field (§7.19 notes)|Yes|B4 Appendix A includes it; B4 §6.2 EMIT block does not. Confirm correct payload.|
|OI-10|Audit scope for `workflow.step.completed` — all step types, or only `approval` and `multi_referral` as B4 implies (§7.12 notes)|Yes|B2's law says all events → Audit. Confirm if B4's annotation is intentional scoping or a documentation omission.|
|OI-11|`workflow.sla.warning` and `workflow.sla.critical` consumer list — Notifications subscription is `[Inference]` (§7.27, §7.29)|Yes|Confirm whether Notifications subscribes to warning and critical in addition to breached.|
|OI-12|`documents.certification_urgency.logged` missing from B2 Master Registry — B2 must be updated|Yes|Add the event to B2 Master Registry (row 13) in the same PR that introduces the event.|
|OI-13|`workflow.instance.created` `documentType` field — full enum of workflow-capable document types|No|Refine `z.string()` to a proper enum once all document types that can have workflow instances are finalized.|
|OI-14|`workflow.instance.completed` `outcomeCode` field — exact outcome code values|No|Define the enum of outcome codes (e.g. `PUBLISHED`, `ARCHIVED`, `VETO_OVERRIDE_FAILED`, `REPASSED`) once the workflow engine's termination step configurations are finalized.|
|OI-15|`workflow.step.completed` `outcome` field — exact outcome values per step type|No|Define per-step-type outcome enums and replace `z.string()` with a discriminated union once step type configurations are finalized.|
