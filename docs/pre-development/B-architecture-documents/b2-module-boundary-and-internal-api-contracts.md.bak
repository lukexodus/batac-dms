...
interface DocumentsPublicAPI {
   ...
   * cover sheet generation; and by Search Meta (Phase 2) for indexing.
   * Returns null if not found.
   * [Inference]
   */
...
interface NotificationsPublicAPI {
   ...
   * formal written notices to complaint respondents (confirmed in ADR-B2-4).
   *
   * Note: Portal's Respondent Notice Service routes through this method
   * instead of calling SMTP directly, so all notification delivery is logged
   * in a single delivery_log.
   */
...
interface SearchMetaPublicAPI {
   ...
   * Phase 1: delegates to PostgreSQL FTS.
...
|Portal (respondent notice service)|Notifications|`sendNotification()`|Formal written notice to complaint respondent|ADR-B2-4|
...
Documents
  ...
  Emits to:       Tracking, Workflow, Notifications, Search Meta [Ph2],
                  Portal [Ph3], Audit
...
Workflow
  ...
  Consumes from:  Documents (created, secretariat_decision)
...
Search Meta
  ...
  Emits to:       (none)
  Consumes from:  Documents (created, state_changed)
...
The following architectural decision points identified in this document require a dedicated ADR before the affected feature is implemented.
...
| ADR-B2-2 | Audit Log Design                         | Hash chain algorithm details, HMAC key storage and rotation, tamper-evident vs. tamper-proof boundary statement, TSA provider selection and export schedule. The "tamper-evident, not tamper-proof" claim must be stated verbatim in the ADR.                                                                            |
...
| ADR-B2-4 | Respondent Notice Channel                | **Resolved (Accepted):** Portal's Respondent Notice Service routes through `Notifications.sendNotification()`. B1's direct-SMTP diagram is superseded. (See ADR-B2-4 below). |
| ADR-B2-5 | Phase 1 FTS Column Ownership             | Confirm whether the `tsvector` FTS column lives in the `documents` schema (maintained by DB trigger, no Search Meta involvement in Phase 1) or requires a Phase 1 Search Meta coordination layer. Affects whether Search Meta module needs any Phase 1 implementation at all.                                            |
| ADR-B2-6 | Published API Versioning and Deprecation | Define how breaking changes to a Published API method are handled before the first inter-module API is deployed to production. Specifically: how is an old method signature deprecated, and how long must it coexist with the new signature.                                                                             |
| ADR-B2-7 | Phase 1 Classification Source            | Confirm that in Phase 1 (before the Records module is active), the IAM ABAC engine reads `classificationLevel` from `Documents.getDocumentById()`. Confirm this field transitions to `Records.getClassificationForDocument()` as the canonical source in Phase 2. Requires a deliberate migration plan at Phase 2 start. |

## Resolved ADRs

# ADR-B2-4: Respondent Notice Channel
...
B1's Component Diagram for the Portal module (Module 10) shows the **Respondent Notice Service calling the LGU SMTP Server directly**: *"Rel(respondentNoticeSvc, smtpServer, 'Sends formal email notice when email address on file')"* (B1, line 561), with the SMTP server itself listed as an explicit external system in B1's Level 1 System Context (*"Delivers step-assignment notifications, overdue alerts, and formal respondent notices when respondent has an email address"*, B1 line 63).

B2 proposed an alternative: route this call through `Notifications.sendNotification()` instead, so that respondent notices — like every other notification in the system — land in the single `delivery_log` owned by the Notifications module, rather than Portal independently calling SMTP and that delivery attempt going unrecorded anywhere centrally. B2 flagged this explicitly as a design decision to confirm (B2, Module 7 Published API doc comment: *"this is a design decision to confirm in the ADR for respondent notices"*).
...
1. When the Portal module's Complaint Ingester routes a complaint and a respondent notice needs to go out, the **Respondent Notice Service calls `Notifications.sendNotification(input)`** with:
   ...
   ```
2. **Phase 1 and Phase 2 behavior when only a contact number is available:** Per the Consolidated Reference (Part 4.14) and B1's Notifications Module 7 note (*"Phase 1 and 2: respondent is called by phone; formal written notice must be claimed in person from LGU"*), there is no SMS gateway yet. `Notifications.sendNotification()` with `channel: 'sms'` in Phase 1/2 does not silently fail — the Notifications module's SMS Delivery Interface (B1, Module 7: *"Reserved — Phase 3"*) is implemented in Phase 1 as a **logging-only stub**: it records a `delivery_log` entry of type `phone_call_required`, and the actual phone call and in-person notice handoff remain manual Secretariat actions tracked outside the system (consistent with B1's existing note that this is a manual process in Phase 1/2). The point of routing through Notifications even for this manual-fallback path is that the *attempt* and its outcome are still logged centrally, rather than Portal needing its own ad hoc tracking for "did we try to notify this respondent."
