...
- [L1255–L1273] 11. Deferred Decisions (Must Resolve Before IAM Module Migration) — Summary of resolutions for the 10 deferred decisions; open follow-ups are superseded by Section 12.
- [L1274–L1288] 12. Remaining Open Items — Four unresolved or follow-up items not blocking IAM migration, detailing what is open and when resolution is required; plus the new item logged 2026-06-25 (office-assignment-uniqueness — see §5.6).
...
| Office-assignment uniqueness `[New, 2026-06-25]` | `organization.assignments` has no constraint that blocks two simultaneous `is_active = true` rows for the same employee, contradicting §5.6 point 3's "every user has a primary record... linking them to one office." Surfaced while resolving the IAM module's `oid`/`cid` JWT claim spec gaps (see `docs/pre-development/A-project-planning/a1-tasks/iam.md` Module Summary) and independently flagged by frontend ADR-UI-012's "Open Follow-Up" (accepted 2026-06-19) — neither resolves it; both only confirm it's real. Three options are laid out in iam.md's Module Summary without a recommendation; this requires an organizational fact (whether any Batac City employee actually holds two positions concurrently) this document cannot supply. | Doesn't block Wave B: the `getPrimaryOffice` resolver is a no-op returning `null` until the ORG module exists, regardless of how many `organization.assignments` rows would eventually exist — Phase 1 has no real query to disambiguate yet. Doesn't block Wave C either once it does exist: ABAC office-scope checks fail closed on a wrong or absent "primary" office either way (under-grants rather than over-grants in the worst case — I1 §3.2's array-membership comparison), so shipping *some* tie-break behavior and refining it later carries low security risk, even though this document does not specify what that initial tie-break should be. | Before the ORG module's Step 2 pass writes `organization.assignments`' DDL migration, if a schema constraint (option a in iam.md) is the chosen resolution — otherwise, before real multi-assignment data could exist in production |

---

_This document is the pre-development baseline for the authentication and authorization architecture. As of this revision, 8 of the 10 items originally listed in Section 11 are resolved and reflected in the relevant body sections above; the remaining 2 fully open items (D-AUTH-08) plus 3 narrower follow-ups on otherwise-resolved items (D-AUTH-02, D-AUTH-05, D-AUTH-07) are tracked in Section 12; [ADR-AUTH-008](b5-authentication-and-authorization-architecture-adrs/ADR-AUTH-008-external-tsa-provider-for-audit-log-timestamps.md) above, each with an explicit account of why it does not block the IAM module's first migration. A fifth item — office-assignment uniqueness — was added to Section 12 on 2026-06-25, discovered while resolving the IAM module's `oid`/`cid` JWT claim spec gaps; it was not one of the original 10 items in Section 11 and has no corresponding ADR yet. Remaining `[Inference]` items elsewhere in this document still require development team confirmation. This document supersedes any earlier auth/auth notes and is the reference for the IAM module schema design._
