...
- [L313–L427] 4. Session Management
  ...
  - [L329–L356] 4.2 Session Table Schema [Inference — not confirmed] — not confirmed] — not confirmed] — PostgreSQL schema for iam.sessions with partial unique index to enforce single active session.
...
- [L916–L1030] 8. Platform Administrator Role Exclusion Invariant
  ...
  - [L928–L949] 8.3 Definition of Document-Processing Roles [Resolved for seeding — ADR/D-AUTH-05] — ADR/D-AUTH-05] — ADR/D-AUTH-05]
  - [L950–L1030] 8.4 Enforcement — TypeScript application-level validations and database-level trigger code to block illegal role combinations.
- [L1031–L1104] 9. Future SSO Migration Path — Design choices for OAuth/OIDC compatibility, external identity mapping column additions, and token exchange flow.
- [L1105–L1260] 10. Implementation Notes
  - [L1107–L1141] 10.1 Fastify Plugin Structure [Inference] — Verification hooks, database session variable setup, and public route configurations.
  - [L1142–L1169] 10.2 tRPC Context [Inference] — Definition of AuthContext and Context TypeScript types used to supply information to ABAC evaluators.
  - [L1170–L1196] 10.3 Audit Events for Authentication and Authorization Actions — Payload fields for 17 auditable events covering logins, logouts, role changes, and ABAC denials.
  - [L1197–L1212] 10.4 Rate Limiting — IP-based and session-based request limits per minute/hour for login, logout, and password resets.
  - [L1213–L1233] 10.4.1 Account-Level Lockout Policy [Resolved — [ADR-AUTH-007](b5-authentication-and-authorization-architecture-adrs/ADR-AUTH-007-account-lockout-policy-on-repeated-login-failures.md); one value still open] — [ADR-AUTH-007](b5-authentication-and-authorization-architecture-adrs/ADR-AUTH-007-account-lockout-policy-on-repeated-login-failures.md); one value still open] — [ADR-AUTH-007](b5-authentication-and-authorization-architecture-adrs/ADR-AUTH-007-account-lockout-policy-on-repeated-login-failures.md); one value still open] — Progressive delays (up to 15 minutes) for repeated login failures instead of hard lockout.
  - [L1234–L1260] 10.5 MFA Readiness: Phase 1 Design, Phase 2 Activation — Phase 1 flow structure supporting environment-gated TOTP validation in Phase 2.
- [L1261–L1280] 11. Deferred Decisions (Must Resolve Before IAM Module Migration) — Summary of resolutions for all tracked decisions including D-AUTH-11 (added and resolved 2026-06-26); open follow-ups are superseded by Section 12.
- [L1281–L1295] 12. Remaining Open Items — Three follow-up items and one fully-open item not blocking IAM migration; the fifth item (office-assignment-uniqueness) was resolved on 2026-06-26 and moved to Section 11 as D-AUTH-11.
...
**Follow-up this resolution connects to, already carried forward elsewhere — not newly surfaced here:** Invariant #12 ("Platform Administrator role cannot be combined with any document-processing role") is enforced by `trg_enforce_platform_admin_exclusion` against a user's own seeded role. `[Corrected — this note originally claimed the delegation/Platform-Admin interaction question was "not yet addressed by any document." That was wrong: I1 §15/§16.2 and I3 §18.2 already carried this exact question forward as an open item, in more detail than this note had — I1 even names the real trigger. This B5 note and I1/I3's copies are now synced to the same resolved/open split rather than presenting this as three separate findings.]` It is not yet defined whether the trigger — which operates on `iam.role_assignments` rows only — needs a delegation-aware enforcement path: a Platform Admin could, as currently specified, become the routing target for Mayor's document-processing steps via delegation while keeping their Platform Admin role, since they'd never acquire a document-processing role row this trigger would catch. Whether this is intentionally out of scope (relying on I1 §16.2's statement that delegation cannot grant Platform Administrator operational access) or requires a delegation-aware path is the open question — see I1 §15's Open Items table and I3 §18.2 for the fuller framing. `[Speculation on the resolution direction — the existence of the gap itself is [Confirmed], independently identified in both this document and I1/I3]`
