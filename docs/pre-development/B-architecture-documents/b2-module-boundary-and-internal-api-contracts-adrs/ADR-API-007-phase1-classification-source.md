# ADR-API-007: Phase 1 Classification Source
...
- **Negative (accepted):** `classificationLevel` exists as a column in both the `documents` and `records` schemas after Phase 2, with the `documents` copy becoming a frozen snapshot rather than being kept live. This is a deliberate, documented duplication rather than an oversight — removing it is possible later under ADR-API-006's same-PR breaking-change process if judged worthwhile, but is out of scope here.
