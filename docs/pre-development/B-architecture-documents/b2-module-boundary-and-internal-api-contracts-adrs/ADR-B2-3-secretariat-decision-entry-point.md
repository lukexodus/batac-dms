# ADR-B2-3: Secretariat Decision Entry Point
...
5. On success, the **Workflow module emits `workflow.step_completed`** (already an existing event in B2's Master Event Bus Registry, with `outcome` carrying the Approve/Reject/Amended result) rather than a new or renamed event. **`document.secretariat_decision` is removed from B2's event taxonomy** — it is no longer needed, since the decision no longer originates in Documents and is no longer the trigger for Workflow's action; the causality is now the reverse (Workflow Router → Workflow Engine → `Documents.transitionState()` → Workflow emits `workflow.step_completed`).
...
- **No change to `workflow.step_completed`:** this event already existed and already carries an `outcome` field; it now additionally covers the Approve/Reject/Amended outcome explicitly, and its existing consumers (Tracking, Audit) require no change.
