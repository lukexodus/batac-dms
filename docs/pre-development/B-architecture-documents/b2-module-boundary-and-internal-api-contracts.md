...
interface NotificationsPublicAPI {
   ...
   *
   * Note: B1 shows Portal's respondentNoticeSvc calling the SMTP server
   * directly. This document proposes routing those calls through this
   * method instead, so all notification delivery is logged in a single
   * delivery_log. This is a design decision to confirm in the ADR for
   * respondent notices. [Inference]
   */
...
|Portal (respondent notice service)|Notifications|`sendNotification()`|Formal written notice to complaint respondent|`[Inference]` (see ADR note)|
...
|ADR-B2-4|Respondent Notice Channel|Confirm whether Portal's Respondent Notice Service calls SMTP directly (as shown in B1's component diagram) or routes through `Notifications.sendNotification()` (as proposed in this document for unified delivery logging). B1 shows a direct SMTP call; this document proposes routing through Notifications.|
