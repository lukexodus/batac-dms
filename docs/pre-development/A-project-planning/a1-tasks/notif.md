...
|---|---|---|---|
| 1 | Recipient identity column: `recipient_employee_id` (FK → `organization.employees`) vs `recipient_user_id` (FK → `iam.users`) | C1 Part 9's literal DDL names the column `recipient_employee_id`. Four independent, mutually-consistent sources say otherwise: E1's `listMine`/`markAsRead` ABAC conditions and `listDeliveryLogs` output schema all reference `recipient_user_id`/`recipientUserId`; B2's `IAMPublicAPI.getUserById(userId)` doc comment states it is "Called by ... Notifications (recipient addressing)"; B2's own `NotificationInput.recipientUserId?: string`; and the upstream `workflow.step.started.assignedTo` payload field (B3 §7.11) is itself a `user_id`, confirmed by WF's own assignee-resolution algorithm returning `{ user_id, resolved_via }[]`. | **Adopted `recipient_user_id` (FK → `iam.users.id`) throughout.** Applied in TASK-NOTIF-001's DDL (explicitly marked as a correction to C1), and consistently in every subsequent task. C1 Part 9 needs a companion column-rename edit — flagged in TASK-NOTIF-001's AI Prompt as an action item for whoever owns C1, mirroring exactly how WF's own Module Summary flagged comparable DDL-vs-other-document conflicts (e.g., D3 vs C1/B4 enum values). |
...
2. **`recipient_user_id` vs `recipient_employee_id` (Conflict #1) checked against four independent sources**, not resolved on a single document's say-so: E1 Module 8 (3 of 4 procedures), B2's `IAMPublicAPI.getUserById` doc comment, B2's own `NotificationInput` interface, and B3 §7.11's `assignedTo` field type via WF's own confirmed assignee-resolution return shape.
