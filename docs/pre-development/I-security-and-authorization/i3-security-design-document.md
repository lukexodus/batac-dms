...
One item is entirely unresolved; three otherwise-resolved items each carry a narrower follow-up; a fifth item, added 2026-06-25, is a freshly-discovered gap rather than a follow-up to one of the original 16. None of the five block the IAM module's first migration — the reason is stated explicitly for each so this is not mistaken for a blanket deferral.
...
| Office-assignment uniqueness `[New, 2026-06-25]` | `organization.assignments` has no constraint that blocks two simultaneous `is_active = true` rows for the same employee, contradicting B5 §5.6 point 3's "every user has a primary record... linking them to one office." Surfaced while resolving the IAM module's `oid`/`cid` JWT claim spec gaps (`docs/pre-development/A-project-planning/a1-tasks/iam.md` Module Summary) and independently flagged by frontend ADR-UI-012 (accepted 2026-06-19). Mirrors the row of the same name in B5 §12. | The `getPrimaryOffice` resolver is a no-op returning `null` until the ORG module exists, so Wave B is unaffected; ABAC office-scope checks fail closed on a wrong or absent "primary" office either way (under-grants rather than over-grants in the worst case — I1 §3.2). | Before the ORG module's Step 2 pass writes `organization.assignments`' DDL migration, if a schema constraint is the chosen resolution — otherwise, before real multi-assignment data could exist in production |
