...
- [L1365–L1406] 18. Security Decisions — Resolution Status — Resolution Status — Resolution logs for 15 previous security decisions and follow-up paths for 4 open items.
- [L1407–L1418] Appendix A: Regulatory Reference Summary — Cross-reference table mapping Data Privacy, ARTA, Local Government Code, and COA obligations to platform controls.
- [L1419–L1461] Appendix B: Security Controls Quick Reference — Quick-reference matrix indexing all security controls, categories, and their specific architectural enforcement points.
...
| 5 | D-AUTH-05 | **Resolved:** `dept_encoder`, `dept_approver`, `sp_secretary`, `sp_member`, `sp_presiding_officer`, `mayor`, `brgy_encoder`, `brgy_captain` seeded with `type_code = 'document_processor'`; `records_officer`, `auditor`, `sys_admin`, `citizen` seeded otherwise. "Acting Mayor" / "OIC (any)" confirmed not to be roles at all — conferred via `delegation_grant` instead (B5 §8.3). Delegation-aware trigger question remains open — see Section 18.2. | B5 §8.3; I1 §15 (Invariant #12) |
...
| D-AUTH-05 / D-ABAC-01 follow-up `[Half-resolved]` | "Acting Mayor"/"OIC (any)" confirmed not to be roles at all — B5 §8.3 now establishes they're conferred via `organization.delegation_grants` (consolidated reference Part 11.13), not a seeded `iam.roles` row of any kind. What remains open: `trg_enforce_platform_admin_exclusion` operates on `iam.role_assignments` only, so a Platform Admin who becomes an Acting-Mayor delegation recipient acquires no role row this trigger catches — whether this is intentionally out of scope (relying on I1 §16.2's statement that delegation cannot grant Platform Administrator operational access) or requires a delegation-aware enforcement path has not been decided. | The trigger logic operates on `type_code`, not on specific role names — the trigger and migration can be written now. This only affects delegation enforcement design, not the schema or trigger function. | Before delegation grants for Mayor-equivalent authority go live in production |
