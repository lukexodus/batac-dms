...
| Refresh Token | HTTP-only cookie `batac_rt` | [UNRESOLVED DECISION — D-AUTH-03: recommended 7 days] | Opaque; stored hashed in `iam.refresh_tokens`; one-time use; rotated on every use |
...
- Stored as a hash in `iam.refresh_tokens`. [UNRESOLVED DECISION — D-AUTH-04: Argon2id vs SHA-256 for refresh token hashing]
...
**External timestamp:** Monthly export to an RFC 3161 TSA (provider is [UNRESOLVED DECISION — D-AUTH-08]) provides external anchoring of the chain state at regular intervals. This extends the tamper-evidence guarantee to cover bulk deletion of recent records — a deletion between two monthly anchors is detectable by comparing the reconstructed chain against the TSA timestamp. [CONFIRMED — Consolidated Ref. Part 11.11]
...
[UNRESOLVED DECISION — specific limits from B5 §10.4 are proposed; must be tuned against observed production traffic before go-live]

After exceeding the login limit: the account is rate-limited from that IP for 15 minutes. The account is not globally locked (preventing denial-of-service lockout attacks). A notification is sent to the account owner. The rate-limit event is audit-logged. IT Admin can clear the rate-limit record manually.

[UNRESOLVED DECISION — D-AUTH-07: Whether to add account-level lockout (N failures from any IP) in addition to IP-based rate limiting]
...
## 18. Open Security Decisions

The following items are explicitly unresolved in the source documents and must be formally decided and recorded in Architecture Decision Records (ADRs) before the relevant module is developed. All are drawn from B5 §11 and I1 §18.

| # | Decision ID | Description | Security Impact | Recommended Decision | Owner | Required Before |
|---|---|---|---|---|---|---|
| 1 | D-AUTH-01 | **JWT signing algorithm: HS256 vs. RS256** | RS256 enables public key verification by external services (important for future SSO relying-party setup). HS256 is simpler. | RS256 — aligns with SSO migration path (B5 §9); public key distribution does not expose signing key | Backend lead | First IAM migration |
| 2 | D-AUTH-02 | **Argon2id parameters (m, t, p)** | Must meet OWASP guidance (≥ 19ms per hash) without degrading login throughput on target server hardware | Benchmark on production hardware; starting point: `m=65536, t=2, p=1` | Infrastructure + backend lead | First IAM migration |
| 3 | D-AUTH-03 | **Refresh token lifetime** | Longer lifetime = better UX but greater exposure window for stolen tokens | 7 days recommended for government intranet with infrequent access; shared workstation lock reduces exposure | Backend lead | First IAM migration |
| 4 | D-AUTH-04 | **Refresh token storage hash algorithm: Argon2id vs. SHA-256** | For 32-byte random tokens, SHA-256 with salt provides sufficient entropy; Argon2id adds brute-force resistance if DB is compromised | SHA-256 is sufficient for 32-byte random tokens; acceptable tradeoff for performance | Backend lead | First IAM migration |
| 5 | D-AUTH-05 | **Full list of document-processing roles for Platform Admin exclusion DB trigger** | DB trigger `trg_enforce_platform_admin_exclusion` checks `type_code = 'document_processor'`; the mapping of all named roles to this type code must be seeded before the trigger can function | Map all roles in Section 4.3 (except `plat_admin`, `sys_admin`) to `document_processor` type | Backend lead + PM | IAM seed data |
| 6 | D-AUTH-06 | **`delegation_grant.scope` field schema** | Step 7d of the ABAC cascade cannot be fully specified without knowing the scope structure (roles[], office_ids[], action types) | JSON object: `{ roles: string[], office_ids: string[], action_types: string[] }` — subject to team review | Backend + DB lead | Organization module migration |
| 7 | D-AUTH-07 | **Account-level lockout policy on repeated login failures** | IP-based rate limiting (D-AUTH-01) handles IP throttling; account-level lockout adds protection against distributed attacks but creates denial-of-service risk | Implement soft lockout (warning + notification) at N failures from different IPs; hard lockout only on confirmed attack pattern; threshold TBD | Security lead | First IAM migration |
| 8 | D-AUTH-08 | **External TSA provider for audit log timestamps (RFC 3161)** | Provider selection affects the legal standing of the external timestamp anchor; some providers offer qualified timestamps | Evaluate government-available TSA providers in the Philippines; consider eTSA or equivalent | IT Director + legal | Pre-production |
| 9 | D-AUTH-09 | **RLS policy expression for `has_cross_office_read_grant()` function** | Cross-office metadata read grants for Records Officer and Auditor must be modelled as either role-based grants or a dedicated grants table; the function referenced in B5 §6.5 must be defined | Implement as role-based check (if subject holds `records_officer` or `auditor` role → grant cross-office read); dedicated grants table if more granular control is needed | DB lead | Documents module migration |
| 10 | D-AUTH-10 | **Session `locked_at` behavior when access token expires while locked** | If user locks screen and access token expires before they return: (a) auto-refresh token, or (b) require full re-login | Option (b) — require full re-login for expired token; adds security for shared workstations; slight UX cost is acceptable | Frontend + backend lead | Frontend login flow |
| 11 | D-ABAC-01 | **Full list of `document_processor` role type codes for Invariant #12 trigger** | All roles in Section 4.3 that are document-processing roles need to be tagged `'document_processor'` type in the IAM seed data | All roles except `plat_admin` and `sys_admin` are document-processing roles; see Section 4.3 | Backend lead | IAM seed data |
| 12 | D-ABAC-02 | **Explicit allowlist structure for Confidential/Restricted document types** | Gate 4 checks an explicit allowlist; the schema for storing and querying that allowlist per `document_type_id` must be defined | Dedicated `document_type_classification_allowlist` table: `(document_type_id, role_id)`; Platform Admin manages via Tier 2 config | DB + backend lead | Documents module migration |
| 13 | D-ABAC-03 | **`has_cross_office_read_grant()` PostgreSQL function definition** | RLS policies reference this function; the grants table and function must be defined consistently with D-AUTH-09 above | Implement as role-type check function initially; upgrade to grants table if needed | DB lead | Documents module migration |
| 14 | D-ABAC-04 | **Audit event `resource_office_id` field** | I1 §8.3 requires filtering audit events by office; the audit event schema must include a resolvable reference to the affected resource's owning office | Add `resource_office_id UUID` column to `audit.events`; populate from document.office_id at audit write time | Backend + DB lead | Audit module schema |
| 15 | D-ABAC-05 | **Definition of "final approval step" in workflow definition** | Invariant #13 (encoder ≠ final approver) requires identifying which step is the final approval step at enforcement time | Add `is_final_approval_step: boolean` property to the workflow step definition schema; validated at workflow definition publish time | Backend lead | Workflow engine schema |
| 16 | D-ABAC-06 | **SP Member committee membership lookup at policy evaluation time** | Several I1 policies require `subject is a confirmed member of committee X`; the data source and query performance must be confirmed | Pre-cache committee membership in the JWT `committees[]` claim at token issue time; refresh on committee reassignment | Backend lead | IAM / Organization module |
...
*This Security Design Document supersedes any earlier security notes or assumptions from prior architecture phases. It is the authoritative reference for all security-related implementation decisions. All `[UNRESOLVED DECISION]` items in Section 18 must be formally decided and recorded in ADRs before the relevant module's first database migration is written. This document must be updated when new resource types, roles, or compliance requirements are introduced.*
