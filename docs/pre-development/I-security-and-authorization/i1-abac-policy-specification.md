...
- [L709–L896] 6. Resource Type: `workflow_step_instance`
  ...
  - [L742–L766] 6.2 `step_instance:complete_action` (action step — encoder/operational) — encoder/operational) — Execution rules for action-type steps, restricting encoder roles to steps they created or are explicitly assigned.
  ...
  - [L852–L865] 6.7 `step_instance:advance` (manual override — skip missing committee report) — skip missing committee report) — Manual override rules allowing the SP Secretary to bypass pending committee reports with mandatory audit-logged justifications.
...
- [L1485–L1635] 15. Architectural Invariants — Complete Formal Statement — Complete Formal Statement
  - [L1489–L1500] Invariant #2 — No Hard Deletes — No Hard Deletes — Absolute system-wide prohibition of hard database deletes, enforcing soft-delete only across all resources and subjects.
  - [L1501–L1516] Invariant #3 — Audit Log Write Isolation — Audit Log Write Isolation — Enforcement of write access limits to the audit log, restricting write operations solely to the audit service.
  - [L1517–L1526] Invariant #8 — Tenant Isolation — Tenant Isolation — Mandatory isolation check denying cross-city resource requests based on matching subject and resource city identifiers.
  - [L1527–L1539] Invariant #9 — Final Number Immutability — Final Number Immutability — Absolute immutability of assigned final numbers, enforced via application checks and database-level constraints.
  - [L1540–L1556] Invariant #10 — IT Admin Document Content Isolation — IT Admin Document Content Isolation — Content isolation rules blocking IT Administrators from reading or exporting confidential/restricted document content, versions, or OCR text.
  - [L1557–L1587] Invariant #12 — Platform Administrator Role Exclusion — Platform Administrator Role Exclusion — Trigger-enforced prohibition against combining Platform Administrator and operational document-processor roles, with noted acting-mayor delegation gaps.
  - [L1588–L1613] Invariant #13 — Encoder Cannot Be Final Approver — Encoder Cannot Be Final Approver — Segregation of duties rule preventing a document's creator or submitter from executing its final workflow approval step.
  - [L1614–L1635] Invariant #16 — One Active Designation Per Person — One Active Designation Per Person — Uniqueness constraint preventing multiple concurrent delegation grants per user, enforced by database partial unique index.
- [L1636–L1688] 16. Delegation Grant Scope Expansion — Scope expansion mechanics for designations, listing non-overridable constraints, auto-expiry behaviors, and Step 7d scope narrowing rules.
- [L1689–L1714] 17. State-Action Compatibility Matrix — Grid mapping valid actions per document lifecycle state, including numbering promotion rules and disposed document audit visibility.
- [L1715–L1731] 18. Deferred Decisions (Resolved This Revision) — Tracking table documenting the resolutions and document locations for six previously deferred design decisions (D-ABAC-01 through 06).
- [L1732–L1742] 19. Remaining Open Items — Table tracking unresolved items, specifically the delegation-exclusion trigger gap, along with its resolution urgency and impact.
...
**Carried-forward gap, half-resolved:** This document's "role category" framing of "Acting Mayor" and "OIC (any)" is now resolved — B5 §8.3 has been updated: neither is a role at all. Per the consolidated-architecture-and-requirements-reference-iteration-3.md Part 11.13 and Architectural Invariant #16, acting/OIC authority is conferred through an `organization.delegation_grants` record (one active designation per person), which reroutes the original authority's workflow-step assignments to the designated person without granting them a new role row. There was never a real "one seeded role vs. several office-specific roles" question — it isn't modeled as a role.

**What remains genuinely open** (this half is not resolved by the above, and is the same question B5 §8.3 itself now separately flags): whether `trg_enforce_platform_admin_exclusion` — which operates on `iam.role_assignments` rows only — needs to become delegation-aware. Since acting/OIC authority is conferred via `organization.delegation_grants`, not a role row, a Platform Administrator who becomes the designated recipient of a Mayor-equivalent delegation is not caught by this trigger at all under the current design, since they never acquire a `document_processor`-type role row. Whether that's the intended final behavior (relying instead on whatever Section 16.2 already says about delegation not overriding Platform Administrator's operational restrictions) or whether the trigger needs a second, delegation-aware enforcement path is the open question. **This is not required before IAM seed data is written** but should be resolved before delegation grants for Mayor-equivalent authority go live in production.
...
|---|---|---|---|
|D-ABAC-01 follow-up `[Half-resolved]`|"Acting Mayor"/"OIC (any)" are confirmed **not** roles at all — see B5 §8.3 (delegation_grant mechanism, consolidated reference Part 11.13). What remains open: whether `trg_enforce_platform_admin_exclusion` (operates on `iam.role_assignments` only) needs a delegation-aware enforcement path, since a Platform Admin who becomes an Acting-Mayor delegation recipient acquires no role row this trigger would catch. Not a duplicate finding — same open question as B5 §8.3's own follow-up note.|The seed list for direct role assignment (the eight roles above) is unaffected by how this gets resolved, and the trigger as defined functions correctly against `iam.role_assignments` regardless.|Before delegation grants for Mayor-equivalent authority go live in production|
