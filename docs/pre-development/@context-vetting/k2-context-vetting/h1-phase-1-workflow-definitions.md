# Phase 1 Workflow Definitions — Filtered Extract for K2 Test Suite Design

**Source document:** H1 ("Phase 1 Workflow Definitions — Structured Data — Blocking") **Platform:** Batac City LGU Platform **Purpose of this extract:** [Inference] This is a filtered copy of H1, reduced to the content needed as input context for K2 ("Workflow Engine Test Suite Design — Pre-dev"). The actual K2 test-case document is not yet written; this extract is preparatory context-gathering only. **Last Updated:** June 2026 **Audience:** Backend development team **Source Documents (of the original H1):** `consolidated-architecture-and-requirements-reference-iteration-3.md` (Post-Interview 2, developer decisions incorporated); `b4-workflow-engine-specification.md` (B4); `d3-state-machine-diagrams.md` (D3)

---

## Note on This Extract

[Unverified — scope note, not present in original H1] This file was produced by reviewing the complete original H1 document (2,541 lines) and retaining only material relevant to the 8 topics named for K2: valid step transitions, invalid step transitions, multi-referral completion conditions, Thursday cutoff enforcement, the Certified Urgent bypass path, the 10-day Mayor lapse timer, the 30-day Panlalawigan timer, and the one-active-designation-per-person constraint, plus version pinning behavior.

**Coverage gap found during filtering — flagged, not resolved:** Of those 8 topics, **6 are covered by H1** (all except version pinning and the designation constraint). The remaining 2 topics were checked for and **not found anywhere in the original H1 document**:

- **Version pinning behavior** (instance resolving steps from its pinned definition version, not the current one) — H1 mentions only one related fragment, in §2.1, about step-key stability across versions and "Option B in-flight migration." That fragment did not describe pinning test scenarios itself, so it has been trimmed from this extract (see §2.1 below). The substantive version-pinning behavior appears to be specified elsewhere (the document calls it out as B4's territory: "the workflow engine execution model (covered in B4)" — original §"About This Document"), not in H1.
- **One-active-designation-per-person constraint** — H1 contains only the `delegation_aware:` role-resolution shorthand (used to resolve Vice Mayor/Mayor steps to an Acting officeholder), which is retained below because it's part of step definitions K2 does need. It does **not** contain the constraint itself (the DB partial unique index / one-designation-at-a-time rule). That appears to live in the consolidated architecture reference (Part 11.13, Part 12 invariant #16), a different source document.

**[Inference]** Whoever writes K2 will likely need to pull those two topics from B4 and/or the consolidated reference directly, since this extract (and its H1 source) does not contain them.

No other exclusions were made — every other section of H1 was found to be substantively relevant to the 6 confirmed K2 topics and is retained below in full.
...
Step keys use `snake_case` and must be stable across definition versions. Rename a step key only when the step's semantic meaning has changed substantially. The keys defined here are the authoritative names for the Phase 1 legislative workflows.

[Excluded from this extract — original sentence referenced "Option B in-flight migration (B4 Section 7.3) maps active steps between versions by `step_key`," which is version-pinning/migration content. Per the confirmed gap noted above, version pinning is not covered by H1 and that fragment did not contain enough to test against on its own, so it has been removed here rather than retained as a dangling reference.]
...
_This is a filtered extract of the H1 document for K2 context-gathering purposes only — it does not itself supersede anything, and is not the authoritative H1 source. [Inference] Changes to step keys, outcome codes, or transition rules in the original H1 require an explicit revision entry with the date and the author; changes affecting legally mandated step guards require SP Secretary sign-off — both per the original H1 document, restated here for continuity._

**Reminder of the flagged gap (see top of this document):** This extract does not contain material on (1) version pinning behavior or (2) the one-active-designation-per-person constraint, because the original H1 source document does not contain that material either. [Unverified] where the authoritative source for those two topics is — likely B4 and/or the consolidated architecture reference, but this has not been confirmed by reading those documents in this conversation.