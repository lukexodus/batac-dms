# D4. Domain Class Diagram — Pre-Dev (C1 Filtered)

**Scope:** Phase 1 bounded contexts only · 46 entities  
...
**Filter applied:** Retains only entities and content needed to produce C1 — Full Database Schema DDL for schemas `iam`, `organization`, `documents`, `workflow`, `tracking`, `records`, `notifications`, `audit`. Excluded: `portal` (Phase 3), `search_meta` (Phase 2), `reporting` (Phase 2), `MfaRecord` (Phase 2).

---

## Overview

This document presents the core domain model for the Batac City LGU Platform as a single UML class diagram. It covers all Phase 1 domain entities drawn from the module schema map in Part 9 of the consolidated reference, supplemented by entities identified through stakeholder interviews (June 9 and June 15) and post-interview developer decisions.
...
46 entities across 8 Phase 1 schemas.
...
- `parallel_split` and `parallel_join` are Phase 2 step types. Reserved in the enum only — no Phase 1 implementation.
...
**14 — NotificationEvent recipient.** In Phase 1, a `NotificationEvent` is sent to an `Employee` (internal staff). The domain model's external citizen recipient (`Citizen`) is part of the Phase 3 portal module and is excluded from Phase 1. The `sentTo` association on `NotificationEvent` points to `Employee` only in the Phase 1 schema.
